...
class ResultsScreen extends MusicBeatSubstate
{
	...
	override function create()
	{
        ...
        //done!
        if (!PlayState.isStoryMode || (songPlayed == 'mic-test' || PlayState.toggledPracticeMode && PlayState.didPauseSkippin))
        {
            ...
            if (PlayState.instance.accuracy > 50)
            {
                switch (songPlayed)
                {
                    case 'technical-difficulties':
                        ...
                        music.loadEmbedded(Paths.music('songending/Technical_Difficulties'), true, true);
                        music.volume = 0.5;
                        music.play();
                    default:
                        Conductor.changeBPM(80);
                        music.loadEmbedded(Paths.music('songending/resultScreen'), true, true);
                        music.volume = 0.3;
            ...
            else
            {
                ...
                music.loadEmbedded(Paths.music('songending/To_The_South_Of_Here'), true, true);
                music.volume = 0.1;
                music.play(false);
                fumbled = true;
            }
	...
	override function update(elapsed:Float)
	{
        ...
        {   
            ...
            PlayState.instance.showedResults = false;
        ...
        {
            PlayState.nextStateIsPlayState = true;
            FlxG.resetState();
        }
	...
	public static function CleanUpAfterYoself(path:String): Void
    {
        if (sys.FileSystem.exists(path) && sys.FileSystem.isDirectory(path))
        {
            ...
            for (entry in entries) 
            {
                ...
                {
                    if (entry != 'autosaved-charts')
                    {
                        trace("Deleting " + pathPlusEntry);
                        CleanUpAfterYoself(pathPlusEntry);
                        sys.FileSystem.deleteDirectory(pathPlusEntry);
                    }
                } 
