	...
	public function new(x:Float, y:Float)
	{
		...
		switch (PlayState.instance.causeOfDeath)
		{
			case 'ate-bullet':	
				...
				painOverlay.cameras = [camOverlay];
				painOverlay.screenCenter();
				painOverlay.alpha = 0.75;
				add(painOverlay);
				
				camMain.shake(0.05, 0.5, true, true);
			...
			case 'ate-many-bullets':
				...
				camMain.shake(0.075, 1, true, true);
				FlxTween.tween(painOverlay, {alpha: 0}, 2);
			default:
				camMain.shake(0.025, 0.3, true, true);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (bf.animation.curAnim.name == 'firstDeath')
		{
			if (bf.animation.curAnim.curFrame == 15 && gameOverCheckShit <= 0)
			{
				...
				gameOver1Tween = FlxTween.tween(gameOver1, {alpha: 1, x: (gameOverX - 15) -15, y: gameOver1.height * gameOver1.scale.y / 3}, 1.5, {type: ONESHOT, ease: FlxEase.expoInOut});
				gameOver2Tween = FlxTween.tween(gameOver2, {alpha: 1, x: (gameOverX + gameOver2.width + 30) - 15, y: gameOver2.height * gameOver2.scale.y / 3}, 1.5, {type: ONESHOT, ease: FlxEase.expoInOut});
			}
		...
		if (!isEnding)
		{
			...
			gameOverTimer = new FlxTimer().start(0.5, function(tmr:FlxTimer)
			{
				...
				camMain.fade(FlxColor.BLACK, 3.5, false, function()
				{
					...
					LoadingSubstate.loadAndSwitchState(new PlayState());
				});
			});
		}
		else
