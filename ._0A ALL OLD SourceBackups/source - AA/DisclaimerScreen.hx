...
class DisclaimerScreen extends MusicBeatState
{
	...
	override function create()
	{
		...
		var transitionSprite:FlxGraphic = FlxGraphic.fromAssetKey(Paths.image('trans_dither'));
	...
	function speedrunSkip():Void
	{
		if(Caching.cacheMusic.exists && Caching.cacheMusic.playing)
			Caching.cacheMusic.volume = 0;
		else if(Caching.disclaimerMusic.exists && Caching.disclaimerMusic.playing)
			Caching.disclaimerMusic.volume = 0;
		FlxG.sound.play(Paths.sound('disclaimerSkip'), 0.65);
		add(black);
		FlxG.camera.fade(FlxColor.BLACK, 0, false, function()
		{
			if (!wentOptions)
