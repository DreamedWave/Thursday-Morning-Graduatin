...
class PlayState extends MusicBeatState
{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			//BabyArrow Anim and Offsets
			if (storyPlaylist.length >= 2 && isStoryMode && FlxG.save.data.video)
				babyTimer = 1.5;

			babyArrow.alpha = 0;

			new FlxTimer().start(babyTimer, function(tmr:FlxTimer)
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			else if (curSong != "Mic Test" && curSong == "Finale")
			{
				if (health < 1)
				{
					drums.volume -= health * -1 + 1;
					taiko.volume -= health * -1 + 1;
					choir.volume -= health * -1 + 1;
					hats.volume -= health * -1 + 1;
					adders.volume -= health * -1 + 1;
					slayer.volume -= health * -1 + 1;
					retalHats.volume -= health * -1 + 1;
					bells.volume -= health * -1 + 1;
					pads.volume -= health * -1 + 1;
					finaleDanger.volume = health * -1 + 1;
				}
				else if (health > 1 && FlxG.sound.music.volume < 1)
				{
					drums.volume = 0.8;
					...
					adders.volume = 0.8;
					slayer.volume = 0.7;
					retalHats.volume = 0.9;
					bells.volume = 0.8;
					pads.volume = 0.8;
					finaleDanger.volume = 0.1;
				}
			}
		}
...
//					trace(daNote.sustainActive);
					...
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('Note_Sustain'), 0.3);
					}
					else if (FlxG.save.data.notesfx && !FlxG.save.data.distraction && allowNoteHitSounds)
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('NoteChill_Sustain'), 0.25);
					}
				}
			});
		}
		...
		new FlxTimer().start(0.003, function(tmr:FlxTimer)
		{
			...
			if (!PlayStateChangeables.botPlay || loadRep || FlxG.save.data.distractions)
			{
				...
				switch (rating)
				{
					case 'sick':
						//For Sick Rating
						FlxG.sound.play(Paths.sound('Note_Sick'), noteHitVolume);
					case 'good':
						//For Good Rating
						FlxG.sound.play(Paths.sound('Note_Good'), noteHitVolume);
					case 'bad':
						//For Bad Rating
						FlxG.sound.play(Paths.sound('Note_Bad'), noteHitVolume);
					case 'shit':
						//For Shit Rating
						FlxG.sound.play(Paths.sound('Note_Crap'), noteHitVolume);
				}
			}
			else if (!PlayStateChangeables.botPlay || loadRep || !FlxG.save.data.distractions)
			{
				...
				switch (rating)
				{
					case 'sick':
						//For Sick Rating
						FlxG.sound.play(Paths.sound('NoteChill_Sick'), noteHitVolume);
					case 'good':
						//For Good Rating
						FlxG.sound.play(Paths.sound('NoteChill_Good'), noteHitVolume);
					case 'bad':
						//For Bad Rating
						FlxG.sound.play(Paths.sound('NoteChill_Bad'), noteHitVolume);
					case 'shit':
						//For Shit Rating
						FlxG.sound.play(Paths.sound('NoteChill_Crap'), noteHitVolume);
				}
			}
			else
			{
			...
			switch (curSong)
			{
				...
				case 'Playing With Fire':
					...
					switch (curBeat)
					{
						...
						case 186 | 198 | 210 | 226 | 231 | 235 | 241 | 404 | 410 | 418 | 422 | 429 
				...
				case 'Finalee'://ADAPTIVE MUSIC WOOOO
				//Drums/Taiko
				//Choir, Taiko, Drums, Hats, Bells
				if (curBeat % 16 == 0)
				{
					drums = new FlxSound().loadEmbedded(Paths.soundRandom('finaleAdaptiveMus/drums_', 0, 3));
					taiko = new FlxSound().loadEmbedded(Paths.soundRandom('finaleAdaptiveMus/taiko_', 0, 5));
					drums.volume = 0.8;
					taiko.volume = 0.8;
				}
				...
				if (curBeat % 32 == 0)
				{
					if (dangerLevel < 50)
					{
						choir = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/choir_0'));
						hats = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/hats_0'));
						if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
							adders = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/woodwinds'));
					}
					else
					{
						choir = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/choir_1'));
						hats = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/hats_1'));
						adders = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/violin'));
					}
					if (enemyHealth < 30 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
						slayer = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/slayer_0'));
					else
					{
						slayer = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/slayer_1'));
						if (dangerLevel < 50)
							dangerLevel = 50;
					}
					adders.volume = 0.8;
				...
				if (curBeat % 64 == 0)
				{
					...
					if (dangerLevel < 50 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
						retalHats = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/retal-hats_0'));
					else
						retalHats = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/retal-hats_1'));
					retalHats.volume = 0.9;
					retalHats.play();
				...
				if (curBeat % 64 == 32)
				{
					if (dangerLevel < 50)
					{
						bells = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/bells_0'));
						if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
							pads = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/pads_0'));
					}
					else
					{
						bells = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/bells_1'));
						pads = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/pads_1'));
					}
					bells.volume = 0.8;
					pads.volume = 0.8;
					bells.play();
				...
				if (curBeat % 128 == 0)
				{
					finaleDanger = new FlxSound().loadEmbedded(Paths.sound('finaleAdaptiveMus/danger-loop'));
					finaleDanger.volume = 0.1;
					finaleDanger.play();
				}
			}
