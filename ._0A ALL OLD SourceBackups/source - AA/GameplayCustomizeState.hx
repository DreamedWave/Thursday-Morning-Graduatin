...
class GameplayCustomizeState extends MusicBeatState
{
    ...
    var defaultX:Float = FlxG.width * 0.5 - 120;
    var defaultY:Float = FlxG.height / 2 - 300;
    ...
    public override function create() {
		...
		strumLine = new FlxSprite(50, FlxG.save.data.strumline).makeGraphic(FlxG.width, 14);
        ...
        text = new FlxText(5, -40, FlxG.width - 5, "Click and drag around gameplay elements to customize their positions.\nPress [R] to reset.\nPress [ESCAPE] to go back.", 12);
		text.scrollFactor.set();
		text.setFormat("VCR OSD Mono", 25, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        
        blackBorder = new FlxSprite(-30, -40).makeGraphic((Std.int(text.width + 100)),Std.int(text.height * 1.3),FlxColor.BLACK);
		blackBorder.alpha = 0;
        ...
        sick.x = FlxG.save.data.changedHitX - 65; // - (FlxG.save.data.changedHitX * 0.2);
        sick.y = FlxG.save.data.changedHitY + 10; // - (FlxG.save.data.changedHitX * 0.2);
    ...
    override function update(elapsed:Float) {
        ...
        {
            FlxG.save.data.changedHitX = sick.x + 65;
            FlxG.save.data.changedHitY = sick.y - 10;
            FlxG.save.data.changedHit = true;
        }

        if (FlxG.keys.justPressed.R)
        {
            sick.x = defaultX; // - (defaultX * 0.2);
            sick.y = defaultY; // - (defaultX * 0.2);
            FlxG.save.data.changedHitX = sick.x + 65; // + (sick.x * 0.2);
            FlxG.save.data.changedHitY = sick.y - 10; // + (sick.x * 0.2);
	...
	private function generateStaticArrows(player:Int):Void
        {
            ...
            {
                // FlxG.log.add(i);
                var babyArrow:FlxSprite = new FlxSprite(50, strumLine.y + 100);
