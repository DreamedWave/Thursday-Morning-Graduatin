	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			...
			if (PlayState.SONG.song.toLowerCase() == "mic test")
			{
				...
				FlxTween.tween(FlxG.camera, {x: camera.x - 1000, y: camera.y - 800}, 1.5, {ease: FlxEase.expoIn, startDelay: 2});
			}
			else
			{
				FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.1}, 0.5, {ease: FlxEase.elasticOut});
				FlxTween.tween(FlxG.camera, {y: camera.y + 800}, 2, {ease: FlxEase.expoIn, startDelay: 2});
			}
			FlxG.camera.follow(camFollow, LOCKON, 0.01);
			isEnding = true;
