...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	var sectionsUntilReset:Int = 2; //Threshold for how many sections you want until it counts up one intimidation point or resets
	...
	override public function create():Void
	{
		...
		if (screenshotForNextSong && loadScreenshot)
		{
			trace('hello???');
			screenshotForNextSong = false;
			loadScreenshot = false;
			fakeScreen = new FlxSprite().loadGraphic(FlxScreenGrab.screenshot.bitmapData);
			fakeScreen.screenCenter();
			fakeScreen.setGraphicSize(Std.int(fakeScreen.width / FlxG.width));
			fakeScreen.antialiasing = FlxG.save.data.antialiasing;
			fakeScreen.cameras = [camEXT];
			add(fakeScreen);
		...
		switch (songLowercase)
		{
			...
			case 'disagreement':
				...
				if (isStoryMode && !playedCutscene)
				{
					...
					video.finishCallback = function()
					{
						if (fakeScreen != null && fakeScreen.exists)
	...
	function cacheGameplayAssets():Void
	{
		...
		{
			DC.beginProfile("IndivLoadingShit");
			switch(type)
			{
				case 'image':
					Paths.image(key);
			...
			#end
			DC.endProfile("IndivLoadingShit");
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (!PlayStateChangeables.botPlay && FlxG.keys.justPressed.ONE)
			fakeScreen.visible = !fakeScreen.visible;
	...
	function goodNoteHit(note:Note, ?isJack:Bool = false):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			else if (note.parent.rating != "miss")
				note.rating = note.parent.rating;
    ...
    private function calculateHealth(hpDrainType:Int, ?health:Float = 1, ?accuracy:Float = 90, ?externalValue:Float = 0):Float
    {
        ...
        switch (hpDrainType)
        {
            ...
            case 9:
                feedbackHealth = ((storyDifficulty + 1) / accuracy) * 0.05;
            //Enemy Sing
            case 10:
                switch (storyDifficulty)
                {
	...
	override function beatHit()
	{
		...
		if (curBeat == dummyBeats + 1)
			dummyBeats = curBeat;
		else if (curBeat > dummyBeats + 1 && curBeat != 0 && curBeat != 1)
		{
			trace ('whoop-');
					...
					switch (curSong)
					{
						...
						case "Sudden Confrontation" | "Sprouting Irritation":
							...
							else
							{
								...
								{
									if (missesInSection <= 0 && (notesHitInSection > 4 || isBFTurn))
									{
