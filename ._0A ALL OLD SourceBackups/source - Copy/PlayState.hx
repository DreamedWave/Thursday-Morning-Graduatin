...
class PlayState extends MusicBeatState
{
	...
	public static var preloadedFinaleShit:Bool = false;
	...
	private var hitSoundSick:FlxSound = new FlxSound();
	private var hitSoundGood:FlxSound = new FlxSound();
	private var hitSoundBad:FlxSound = new FlxSound();
	private var hitSoundShit:FlxSound = new FlxSound();
	private var hitSoundSpecial:FlxSound;
	...
	override public function create()
	{
		...
		if (!playedCutscene && storyPlaylist.length > 2 && SONG.song != 'Mic Test' && isStoryMode)
		{
			camHUD.alpha = 0;
			camGame.alpha = 0;
				...
				switch (SONG.player2)
				{
					...
					case "priest-theborderangry":
						if (FlxG.save.data.distractions)
						{
							if (!PlayStateChangeables.Optimize)
							{
								trailBlurEffect = new FlxTrail(dad, null, 3, 8, 0.4, 0.08);
								trailBlurEffect.alpha = 0.5;
								add(trailBlurEffect);
		...
		if (SONG.song == "Playing With Fire")
		{
			hitSoundSpecial = FlxG.sound.load(Paths.sound('Note_Mine'));
			hitSoundSpecial = FlxG.sound.load(Paths.sound('Note_Trigger'));
		}
		else if (SONG.song == "Finale")
		{
			loadFinaleMusic();
			trace("Yuh");
			/*drums.volume = 1;
	...
	function startCountdown():Void
	{
		...
		else
		{
			...
			if (camGame.alpha <= 0 && storyPlaylist.length > 2 && SONG.song != 'Mic Test' && isStoryMode)
			{
				FlxTween.tween(camHUD, {alpha: 1}, 0.15);
				FlxTween.tween(camGame, {alpha: 1}, 0.15);
		...
		if (!skipCountdown)
		{
			...
			startTimer = new FlxTimer().start(0.05);
	}
	...
	private function fadeOutGameplayHUD(delayMultiplier:Float):Void
	{
		...
		strumLineNotes.forEach(function(babyArrow:FlxSprite)
		{
			for (i in 0...4)
			{
				...
				babyArrow.velocity.x -= FlxG.random.int(0, 20); 
				FlxTween.tween(babyArrow, {alpha: 0}, 0.5, {ease: FlxEase.quadOut, startDelay: 0.5 * (delayMultiplier / 2)});
			}
		});
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && startedCountdown && !cannotDie)
		{
			switch (curSong)
			{
				...
				case "Tech. Difficulties" | "Retaliation":
					...
					if (health < 1)
						FlxG.sound.music.volume = health + 0.25;
		...
		else
		{
			...
			if (!paused)
			{
				...
				if (previousFrameTime > FlxG.game.ticks + 50)
				{
					allowHealthModifiers = false;
					trace("Bruh'd");
				}
		...
		if (startedCountdown)
		{
			...
			{
				if (!cannotDie && !resultsPresent && !inCutscene && !PlayStateChangeables.botPlay)
				{
					if (!usedTimeTravel) 
						literallyFuckingDie();
				}
				...
				{
					lowHPHeartBeat = FlxG.sound.play(Paths.sound('lowHP'), 1);
					trace("nope.");
				}
			}
		}
				...
				&& daNote.mustPress)
			{
				if (daNote.isSustainNote && daNote.wasGoodHit)
					...
					else
					{
						...
						{
							hitSoundSpecial = FlxG.sound.play(Paths.sound('Note_Mine'));
							boyfriend.playAnim('scared', true);
		...
		if (holdArray.contains(true) && generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && holdArray[daNote.noteData] && daNote.sustainActive)
				{
					...
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('Note_Sustain'), noteHitVolume);
					}
					else if (FlxG.save.data.notesfx && !FlxG.save.data.distraction && allowNoteHitSounds)
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('NoteChill_Sustain'), noteHitVolume);
					}
				}
			});
		}
	...
	{
		trace ("Shit!!");
		trace ("Preloaded Final Shit is " + preloadedFinaleShit);
		if (deaths <= 0 && !hasReset && !preloadedFinaleShit)
		{
			...
			/*drums = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/drums_1'));
			drums = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/drums_2'));
			drums = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/drums_3'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_0'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_1'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_2'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_3'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_4'));
			taiko = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/taiko_5'));
			choir = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/choir_0'));
			choir = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/choir_1'));
			hats = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/hats_0'));
			hats = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/hats_1'));					
			adders = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/woodwinds'));					
			adders = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/violin'));
			slayer = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/slayer_0'));
			slayer = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/slayer_1'));
			retalHats = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/retal-hats_0'));
			retalHats = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/retal-hats_0'));
			bells = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/bells_0'));
			bells = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/bells_1'));					
			pads = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/pads_0'));
			pads = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/pads_1'));
			danger = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/danger-loop'));
			FlxG.sound.load(Paths.sound('finaleAdaptiveMus/stab'));*/
			trace("Preloaded Finale Shit");
			preloadedFinaleShit = true;
		}
		/*else
		{
			drums = new FlxSound();
			...
			pads = new FlxSound();
			danger = new FlxSound();
			//Check if this increases cache
			trace("Did La Skip Cache Finale Shit");
		}*/
	}

	var preloadHitSFX:Bool = false;
	...
	function loadNoteHitSound():Void
	{
		if (!preloadHitSFX)
		{
			...
			{
				hitSoundSick = FlxG.sound.load(Paths.sound('Note_Sick'));
				hitSoundGood = FlxG.sound.load(Paths.sound('Note_Good'));
				hitSoundBad = FlxG.sound.load(Paths.sound('Note_Bad'));
				hitSoundShit = FlxG.sound.load(Paths.sound('Note_Crap'));
			}
			else if (!PlayStateChangeables.botPlay || loadRep || !FlxG.save.data.distractions)
			{
				hitSoundSick = FlxG.sound.load(Paths.sound('NoteChill_Sick'));
				hitSoundGood = FlxG.sound.load(Paths.sound('NoteChill_Good'));
				hitSoundBad = FlxG.sound.load(Paths.sound('NoteChill_Bad'));
				hitSoundShit = FlxG.sound.load(Paths.sound('NoteChill_Crap'));
			}
			else if (PlayStateChangeables.botPlay && !loadRep)
			{
				hitSoundSick = FlxG.sound.load(Paths.sound('Note_Botplay'));
			}
		}
		/*FlxG.sound.list.add(hitSoundSick);
		FlxG.sound.list.add(hitSoundGood);
		FlxG.sound.list.add(hitSoundBad);
		FlxG.sound.list.add(hitSoundShit);*/
	}
	...
	function playNoteHitSound(note:Note):Void
	{
		...
		//{
		//	SFX for hitting notes (such as SICK, GOOD, BAD, SHIT, and SPECIAL NOTEs)
		hitSoundSick.volume = noteHitVolume;
		hitSoundGood.volume = noteHitVolume;
		hitSoundBad.volume = noteHitVolume;
		hitSoundShit.volume = noteHitVolume;
		if (!PlayStateChangeables.botPlay)
		{
			...
			switch (rating)
			{
				case 'sick':
					//For Sick Rating
					hitSoundSick.play(true);
					//Vocal Shit
					vocals.volume = 1;
				case 'good':
					//For Good Rating
					hitSoundGood.play(true);
					//Vocal Shit
					vocals.volume = 0.85;
				case 'bad':
					//For Bad Rating
					hitSoundBad.play(true);
					//Vocal Shit
					vocals.volume = 0.55;
				case 'shit':
					//For Shit Rating
					hitSoundShit.play(true);
					//Vocal Shit
					vocals.volume = 0.1;
			}
		}
	...
	//(note:Note, resetMashViolation = true)
	{
		...
		if (!note.wasGoodHit)
		{
			...
			switch (note.noteStyle)
			{
				...
				case 'trigger':
					hitSoundSpecial = FlxG.sound.play(Paths.sound('Note_Trigger'), 0.8);
					if (FlxG.save.data.accuracyMod == 0)
					totalNotesHit += 1;
					sicks++;
					if (allowHealthModifiers)
	...
	function camFollowShit()
	{
		...
		if (camFollow.x != boyfriend.getMidpoint().x - 100 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		{
			...
			{
				if (curSong == 'Mic Test' && !PlayStateChangeables.Optimize)
					camTween = FlxTween.tween(FlxG.camera, {zoom: 1.2}, (Conductor.stepCrochet * 4 / 850), {ease: FlxEase.elasticInOut});
	...
	override function beatHit()
	{
		...
		if (generatedMusic)
		{
			...
			if (curSong != "Mic Test")
			{
				if (health < 1 && !midsongCutscene && allowHeartBeatSounds && !inResults && !inCutscene && !cannotDie)
				{
					...
					if (Conductor.bpm <= 300)
					{
						...
						{
							lowHPHeartBeat = FlxG.sound.play(Paths.sound('lowHP'), lowHealthSoundVolume);
				...
				case 'Playing With Fire':
					...
					switch (curBeat)
					{
						...
						case 176:
							...
							if (!playedCutscene)
							{
								FlxTween.tween(avoidBulletTipGraphic, {x: avoidBulletTipGraphic.x - 20, y: avoidBulletTipGraphic.y - 15, alpha: 1}, 0.5, {ease: FlxEase.sineInOut});
							}
						case 180:
							allowHeartBeatSounds = true;
							midsongCutscene = false;
							...
							{
								playedCutscene = true;
								avoidBulletTipGraphic.acceleration.y = 800;
								avoidBulletTipGraphic.velocity.y -= FlxG.random.int(140, 175);
								avoidBulletTipGraphic.velocity.x -= FlxG.random.int(0, 5);
