...
class FlxCamera extends FlxBasic
{
	...
	public function updateFollow(?elapsed:Float):Void
	{
		...
		else
		{
			...
			{
				scroll.x = FlxMath.lerp(_scrollTarget.x, scroll.x, CoolUtil.boundTo(1 - (elapsed * (followLerp * 2.5)), 0, 1)) + scrollShakeX;
				scroll.y = FlxMath.lerp(_scrollTarget.y, scroll.y, CoolUtil.boundTo(1 - (elapsed * (followLerp * 2.5)), 0, 1)) + scrollShakeY;
			}
		}
	}
	...
	function updateShake(elapsed:Float):Void
	{
		if (_fxShakeDuration > 0)
		{
			...
			else 
			{
				...
				{
					if (_fxShakeAxes != FlxAxes.Y)
						flashSprite.x += (FlxG.random.float(-_fxShakeIntensity * width, _fxShakeIntensity * width) / 2) * (decayCamShake ? shakeDecayFactor : 1);

					if (_fxShakeAxes != FlxAxes.X)
						flashSprite.y += (FlxG.random.float(-_fxShakeIntensity * height, _fxShakeIntensity * height) / 2) * (decayCamShake ? shakeDecayFactor : 1);
				}
			}
		}
	}
