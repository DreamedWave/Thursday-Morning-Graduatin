...
class FlxCamera extends FlxBasic
{
	...
	public function updateFollow():Void
	{
		...
		else
		{
			...
			{
				scroll.x += ((_scrollTarget.x - scroll.x) * followLerp * (FlxG.updateFramerate / 60)) + scrollShakeX;
				scroll.y += ((_scrollTarget.y - scroll.y) * followLerp * (FlxG.updateFramerate / 60)) + scrollShakeY;
			}
		}
	}
		...
		if (_fxShakeDuration > 0)
		{
			...
			else if (target != null)
			{
				if (_fxShakeAxes != FlxAxes.Y)
				{
					...
					scrollShakeX = scrollShakeX / 2 / zoom;
				}
				if (_fxShakeAxes != FlxAxes.X)
				{
					if (scrollShakeY < 0)
						scrollShakeY = FlxG.random.float(0, _fxShakeIntensity * width) * (decayCamShake ? shakeDecayFactor : 1);
					else
						scrollShakeY = FlxG.random.float(-_fxShakeIntensity * width, 0) * (decayCamShake ? shakeDecayFactor : 1);
					scrollShakeY = scrollShakeY / 2 / zoom;
				}
			}
			else
			{
				if (_fxShakeAxes != FlxAxes.Y)
				{
					flashSprite.x += (FlxG.random.float(-_fxShakeIntensity * width, _fxShakeIntensity * width) / 2) / zoom;
				}
				if (_fxShakeAxes != FlxAxes.X)
				{
					flashSprite.y += (FlxG.random.float(-_fxShakeIntensity * height, _fxShakeIntensity * height) / 2) * zoom;
				}
			}
		}
	}
