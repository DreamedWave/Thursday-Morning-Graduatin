...
import funkincrew.util.ICloneable;
 ...
 * - Backported by me (Amiee) to whatever the hell more primitive schtuff Kade Engine 1.6 runs on!
 */
@:nullSafety
class FunkinSound extends FlxSound implements ICloneable<FunkinSound>
{
  ...
  {
    if (params.overrideExisting && FlxG.sound.music.exists == true && FlxG.sound.music.playing) 
      return false;

    if (params.restartTrack && FlxG.sound.music.playing)
    {
      if (FlxG.sound.music != null && Std.isOfType(FlxG.sound.music, FunkinSound))
      {
        ...
        if (existingSound._label == Paths.musicString('$key/$key'))
        {
          return false;
        }
      }
    ...
    var music = FunkinSound.load(Paths.musicString('$key/$key'), (params.startingVolume != null ? params.startingVolume : 1.0), (params.loop != null ? params.loop : true), false, true);
    if (music != null)
    {
   ...
   * @return static function construct():FunkinSound
   */
   public static function playLooping(key:String, volume:Float = 1.0, ?onComplete:Void->Void, ?onLoad:Void->Void):Void
    {
      var result = FunkinSound.load(key, volume, true, true, true, onComplete, onLoad);
    }
