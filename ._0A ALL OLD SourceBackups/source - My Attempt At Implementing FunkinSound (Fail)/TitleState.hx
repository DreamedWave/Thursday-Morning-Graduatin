...
import funkincrew.audio.FunkinSound;
		...
		if (!FlxG.sound.music.playing && !didThePress)
		{
			...
			switch (FlxG.save.data.weekUnlocked)
			{
				case 0 | 1:
					musicKey = 'freakyMenu';
				case 2:
					musicKey = 'glitchyMenu';
				case 3:
					musicKey = 'cityMenu';
				case 4:
					musicKey = 'villageMenu';
				default:
					musicKey = 'peacefulMenu';
			}
			FunkinSound.playMusic(musicKey,
			{
			  startingVolume: 0.3,
	...
	function doSpecialWacky():Void
	{
		...
		switch (wackySwitch)
		{
			...
			case 'slmclclveiwerr':
				...
				wackyXtraText.visible = true;
				FlxG.log.add('WackyText');
				FunkinSound.playOnce(Paths.soundString('intro_schplat'), 0.45);
			...
			case 'pleasebecomefunny' | 'lmaoodeadchanneldead' | 'thedreamedwavechannel' | 'bruh' | 'bruuuhhh':
				FunkinSound.playOnce(Paths.soundString('intro_bruh'), 0.55);
			...
			case 'imposterfromamoger' | 'amongusjokes' | 'anentiregeneration' | 'theyweretoosus':
				FlxG.sound.play(Paths.soundString('intro_sus'), 0.55);
			case 'haha' | 'shitsaggressively' | 'bwap' | 'aggressiveshitting' | 'goneinaninstant':
				doCamZooming = false;
				camUI.zoom -= 0.03;
				FunkinSound.playOnce(Paths.soundString('intro_shid'), 0.55);
				FlxG.log.add('BRAAPPP');
			case 'urmomhahagottem' | 'theymademeanegg' | 'afamily' | 'x':
				camUI.zoom += 0.015;
				FunkinSound.playOnce(Paths.soundString('intro_vine'), 0.65);
		...
		switch (pressedWhat)
		{
			case 1:
				if (!transitioning && !failSafeAugh)
				{
					...
					FunkinSound.playOnce(Paths.soundString('confirmMenu' + weekAdderThingy), 0.7);

					FlxG.camera.shake(0.005, 1, true, true);
			...
			case 2:	
				...
				FunkinSound.playOnce(Paths.soundString('exitMenu' + weekAdderThingy), 1);
			...
			default:
				skipIntro();
				FunkinSound.playOnce(Paths.soundString('skipIntro'), 0.9);
		}
	}
}
