	...
	**/
			...
			else if (songStarted && allowHealthModifiers && nearestNoteUpcoming)
			{
				...
				else if (allowHealthModifiers)
				{
					...
					if (mashPresses > mashPressThreshold)
					{
						...
						totalNotesHit += 0.25 - (0.0025 * mashPresses);//Spamming further lowers it
						updateAccuracy();
						#if debug
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.notes[Std.int(curStep / 16)] != null && !showedResults && !endedSong)
		{
			...
			if (!PlayStateChangeables.Optimize && songStarted && generatedMusic)
				camFollowShit();
	...
	private function popUpScore(type:String, daNote:Note = null, ?isJack:Bool = false):Void
	{
		switch (type)
		{
			...
			case 'sustain':
				...
				if (daRating == null)
				{
					trace('gurl FIX THIS WTF???');
					return;
				}
				else
					trace('sus haha rating: ' + daRating);
				...
				if (!PlayStateChangeables.botPlay)
				{
					...
					for (i in seperatedScore)
					{
						...
						numScore.x = FlxG.save.data.changedHitX + (37 * daLoop2);
						numScore.y =  FlxG.save.data.changedHitY + 75;
						numScore.cameras = [camHUD];
						numScore.antialiasing = FlxG.save.data.antialiasing;
						numScore.updateHitbox();
						//guys are more heavy to avoid coverin up the top
						numScore.acceleration.y = FlxG.random.int(400, 450);
						numScore.velocity.y -= FlxG.random.int(90, 120);
						...
						FlxTween.tween(numScore, {alpha: 0}, 0.15, {type: ONESHOT, 
							...
							startDelay: Conductor.crochet * 0.000815
						});
			...
			default:
				...
				if (!PlayStateChangeables.botPlay)
				{
					...
					if (daNote.isParent)
					{
						...
						rating.velocity.y -= FlxG.random.int(90, 120);
					}
					else
					{
					...
					if (daRating != 'miss')
					{
						...
						currentTimingShown.y = rating.y + (daNote.isParent ? 10 : 0) + 95;
					...
					if (showNumShit)
					{
						...
						for (i in seperatedScore)
						{
							...
							if (daNote.isParent)
							{
								numScore.acceleration.y = FlxG.random.int(300, 400);
								numScore.velocity.y -= FlxG.random.int(120, 140);
							}
							else
							{
								numScore.acceleration.y = FlxG.random.int(220, 320);
								numScore.velocity.y -= FlxG.random.int(120, 140);
							}
					...
					if (daRating != 'miss')
					{
						timingTween = FlxTween.tween(currentTimingShown, {alpha: 0}, 0.2, {type: ONESHOT, startDelay: Conductor.crochet * (daNote.isParent ? 0.00125 : 0.001)});
	...
	function updateAccuracy():Void
	{
		...
		trace ('total hit: ' + totalNotesHit + ' | ' + totalNotesHitMax);
	}
