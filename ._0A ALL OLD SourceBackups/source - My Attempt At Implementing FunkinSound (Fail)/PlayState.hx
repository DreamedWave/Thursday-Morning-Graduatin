		...
		switch (type)
		{
			...
			case 'sustain':
				...
				if (!PlayStateChangeables.botPlay)
				{
					...
					for (i in seperatedScore)
					{
						...
						FlxTween.tween(numScore, {alpha: 0}, 0.1, {type: ONESHOT, 
							onComplete: function(tween:FlxTween)
							{
								numScore.destroy();
							},
							startDelay: Conductor.crochet * 0.0008
						});
			...
			default:
				...
				if (!PlayStateChangeables.botPlay)
				{
					...
					if (daRating != 'miss')
					{
						timingTween = FlxTween.tween(currentTimingShown, {alpha: 0}, 0.175, {type: ONESHOT, startDelay: Conductor.crochet * (daNote.isParent ? 0.00125 : 0.001)});
		...
		if (allowHealthModifiers && !daNote.withinCompensation)
		{
			...
			if(FlxG.save.data.missSounds && daNote.noteType != 'trigger')
			{
				...
				if (missSound.playing)
					missSound.stop();
				missSound = FlxG.sound.play(Paths.soundRandom('missnote', 1, 3));
				missSound.volume = sfxVolume * FlxG.random.float(0.5, 0.75);
			}
