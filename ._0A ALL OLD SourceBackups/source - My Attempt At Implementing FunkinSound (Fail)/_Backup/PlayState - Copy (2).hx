...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	function startVideoSubtitles(key:String = ''):Void
	{
		switch (key)
		{
			case 'week2/startcutscene':
				setTimedSubtitle('[Screaming]', 0xFF5ce766, 5.8, 7.1);
				setTimedSubtitle('Oof-', 0xFF5ce766, 7.1, 8.15);
				setTimedSubtitle('W-where are we..?', 0xFF5ce766, 9.1, 10.64);
				setTimedSubtitle('What is this place?!', 0xFF5ce766, 10.64, 12.64);
				setTimedSubtitle('Well, wherever we are, it sure doesnt look safe...', 0xFF5ce766, 12.84, 15.89);
				setTimedSubtitle('..I guess all we can do now is pray to God-', 0xFF5ce766, 18.1, 20.9);
				setTimedSubtitle('Lets hope he hears us', 0xFF5ce766, 20.9, 23.9);
		}
	}
	...
	function setTimedSubtitle(text:String, textColour:FlxColor, ?textBorderColor:FlxColor = FlxColor.BLACK, occurTime:Float, endTime:Float)
	{
		new FlxTimer().start(occurTime, function(tmr:FlxTimer)
		{
			changeSubtitles(text, textColour, textBorderColor);
	...
	function getFuckingShot():Void
	{
		...
		if (allowHealthModifiers)
		{
			...
			gotShotBlurLol.blurX = 5;
			gotShotBlurLol.blurY = 5;
			gotShotBlurVal = 5;
			if (hurtDelay < 8)
				hurtDelay += 2;
			specilNoteSFXGroup.stop();
				...
				if (!PlayStateChangeables.botPlay)
				{
					...
					if (daRating != 'miss')
					{
						...
						if (PlayStateChangeables.botPlay)
						{
							msTiming = 0;
							#if debug
							trace('Botplay Delay Test: ' HelperFunctions.truncateFloat(noteDiff, 5));
							#end
						}
			
						if (currentTimingShown != null)
							grpRatingsMG.remove(currentTimingShown);
						...
						if (currentTimingShown != null)
							grpRatingsMG.add(currentTimingShown);
					}
	...
	function goodNoteHit(note:Note, ?isJack:Bool = false):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			else
			{
				...
				trace (note.daRating)
				songScore += calculateScore(note.daRating, daNote.isSustainNote);
				maxSongScore += calculateScore('sick', daNote.isSustainNote);
				vocals.volume = vocalsVolume;
				if (mashPresses <= mashPressThreshold)
					totalNotesHit += 1;
							...
							case 'Playing With Fire':
								...
								switch (curBeat)
								{
									...
									case 307:
										...
										camShake(true, true, 'camHUD', 0.005, Conductor.crochet * 0.08);
									...
									case 308:
										...
										strumLineNotes.forEach(function(babyArrow:FlxSprite)
										{
											FlxTween.tween(babyArrow, {alpha: 1}, Conductor.crochet * 0.002, {type: ONESHOT, ease: FlxEase.quadIn});
										});
										if (FlxG.save.data.strumline)
											grpStrumLine.forEach(function(leStrussy:FlxSprite)
											{
												FlxTween.tween(leStrussy, {alpha: 0.45}, Conductor.crochet * 0.001, {type: ONESHOT, ease: FlxEase.quadOut, onComplete:
												function(twn:FlxTween)
												{
													doStrumLineBGTweening = true;
												}
							...
							case 'Variegated Skylines':
								...
								if (stageParticles.visible)
								{
									...
									else
									{
										...
										camShake(false, false, 'camHUD', 0.001, idleCamShakeTimer);
									}
									if (curBeat % 2 == 0)
										createStageParticle('theCityPortalOpen/effects/stageParticle');
								}
