...
class DisclaimerScreen extends MusicBeatMenu
{
	...
	override function create()
	{
		...
		if (FlxG.save.data.showPresence)
			DiscordClient.changePresence("[Awaiting Input]", null);
		#end
	}

	var spedran:Bool = false;
	var exitSoundKey:String;
	...
	function exitDisclaimerState():Void
	{
		if (!wentOptions)
		{
			...
			if ((FlxG.save.data.weekUnlocked > 1 && FlxG.random.bool(1)) || Main.aprilFools)
			{
				...
				txt.screenCenter();
				add(txt);
				exitSoundKey = 'preMusic-disclaimer_LAZAZA';
			}
			else
				exitSoundKey = 'preMusic-disclaimer_ENTER';
			FunkinSound.playOnce(Paths.soundString(exitSoundKey));
		...
		{
			exitSoundKey = 'preMusic-disclaimer_ESCAPE';
			FunkinSound.playOnce(Paths.soundString(exitSoundKey));
	...
	function speedrunSkip():Void
	{
		...
		FunkinSound.playOnce(Paths.musicString('preMusic-disclaimer_SKIP'), 0.8);
