...
class PlayState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		curScroll = FlxG.save.data.scrollSpeed;
		...
		defaultScroll = (curScroll == 1 ? SONG.speed : curScroll);
		...
		if (songLowercase != 'finale')
		{
			for (index => section in SONG.notes)
			{
				...
				{
					if ((storyWeek < FlxG.save.data.weekUnlocked || (deaths > 0 || hasReset)) || !isStoryMode)
					{
		...
		scoreTxt = new FlxText(healthBarBG.x - 35 + healthBar.width / 4, healthBarBG.y + 40, 0, Ratings.CalculateRanking(Math.round(dummySongScore), nps, maxNPS, accuracy), 20);
			...
			for(i in 0...unspawnNotes.length)
			{
				...
				else if (dunceNote.strumTime - startTime < 3500)
				{
					...
					if (dunceNote.mustPress)
						dunceNote.y = (playerStrums.members[dunceNote.noteData].y
							+ 0.45 * (startTime - dunceNote.strumTime) * FlxMath.roundDecimal(curScroll,
								2)) - dunceNote.noteYOff;
					else
						dunceNote.y = (strumLineNotes.members[dunceNote.noteData].y
							+ 0.45 * (startTime - dunceNote.strumTime) * FlxMath.roundDecimal(curScroll,
								2)) - dunceNote.noteYOff;
					if (dunceNote.isSustainNote)
						dunceNote.fixPossibleGaps();
					toBeRemoved.push(dunceNote);
				}
			}
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					if (type == 0)
						sustainNote.childBehindParent = true;
					type++;
				}
	...
	private function tweenScroll():Void
	{
		...
		songSpeedTween = FlxTween.tween(this, {curScroll: newScroll}, Conductor.crochet / 1000, 
		{ease: FlxEase.smootherStepOut,
			onComplete: function(twn:FlxTween) 
			{
				sustainHeightMultiplier = FlxMath.roundDecimal(newScroll / prevScroll, 5);

				for (note in notes) 
				{
					//note.dunDidFixinsBool = false;
					note.ratio = sustainHeightMultiplier;
	...
	override public function update(elapsed:Float)
	{
		if (!resultsPresent && !endedSong)
		{
			...
			//better not fuckin lag
			if (songSpeedTween != null && songSpeedTween.active)
			{
				sustainHeightMultiplier = FlxMath.roundDecimal(curScroll / prevScroll, 5);
		...
		if (controls.PAUSE && canPause && !paused && (FlxG.sound.music.time < musicTimeCusp || !FlxG.sound.music.playing))
			pauseGame();
		...
		if (generatedMusic && startedCountdown)
		{
			...
			if (!endedSong && !inResults && !resultsPresent && !startingSong)
			{
				...
				{
					if (!boyfriend.animation.curAnim.name.startsWith('style') || boyfriend.animation.curAnim.curFrame >= 3)
					{
					...
					{
						badStyleHits++;
						if (badStyleHits > 8 && goodStyleHits >= 0)
							goodStyleHits--;
					}
				}
			}
			...
			if (SONG.needsAdaptiveMus)
			{
				...
				switch (songLowercase)
				{
					case 'retaliation':
						if (health < 1)
						{
							FlxG.sound.music.volume = FlxMath.lerp(health - 0.5, FlxG.sound.music.volume, PlayStateHelpers.calculateLerpTime(elapsed, 15, 0, 1));
							miscs.volume = FlxMath.lerp(lowHealthEffectVolume + 0.5, miscs.volume, PlayStateHelpers.calculateLerpTime(elapsed, 15, 0, 1));
			...
			scoreTxt.text = Ratings.CalculateRanking(Math.round(dummySongScore), nps, maxNPS, accuracy, deaths);
		...
		else
		{
			...
			//Health lerp shits
			health = FlxMath.lerp(targetHealth, health, PlayStateHelpers.calculateLerpTime(elapsed, 19, 0, 1));
			Conductor.songPosition += FlxG.elapsed * 1000;
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if (PlayStateChangeables.useDownscroll)
			{
				...
				if (daNote.isSustainNote)
				{
					...
					if (!daNote.mustPress || ((daNote.sustainActive && holdArray[daNote.noteData]) || PlayStateChangeables.botPlay))
					{
				...
				if (daNote.isSustainNote)
				{
					...
					if (!daNote.mustPress || ((daNote.sustainActive && holdArray[daNote.noteData]) || PlayStateChangeables.botPlay))
					{
						...
						if (daNote.y + daNote.offset.y * daNote.scale.y <= center)
						{
							...
							swagRect.bottom = (swagRect.y + (daNote.height * daNote.scale.y + 30)) * sustainHeightMultiplier * Math.sqrt(daNote.scrollSpeed); //basically playing it safe
	...
	function getFuckingShot():Void
	{
		...
		if (allowHealthModifiers)
		{
			...
			if (timesShot <= 2)
			{
				if (targetHealth < 0.7 && timesShot <= 1)
					targetHealth = 0.25;
				else
					targetHealth = 0.05;
			}
		...
		switch (songLowercase)
		{
			case 'up-and-beyond':		
				if (FlxG.save.data.scoreScreen)
				{
					...
					video.finishCallback = function()
					{
						LoadingState.loadAndSwitchState(new MinigameState());
					}
				...
				if (allowHealthModifiers && !daNote.withinCompensation)
				{
					...
					switch (daRating)
					{
						...
						case 'good':
							...
							daRating = 'good';
							//Health Drain/Gain
							targetHealth += PlayStateHelpers.calculateHealth(storyDifficulty, 6, targetHealth, accuracy);
							ss = false;
							goods++;
							if (FlxG.save.data.accuracyMod == 0)
								if (mashPresses <= mashPressThreshold)
						...
						case 'sick':
							...
							//Health Gain
							targetHealth += PlayStateHelpers.calculateHealth(storyDifficulty, 7, targetHealth, accuracy, hurtVignette.alpha);
							if (FlxG.save.data.accuracyMod == 0)
								if (mashPresses <= mashPressThreshold)
									totalNotesHit += 1;
							sicks++;
	...
	private function enemyMiss(noteDirection):Void
	{
		...
		if (!bypassOppMiss)
		{
			...
			if (healthBar.percent < 85)
				targetHealth += PlayStateHelpers.calculateHealth(storyDifficulty, 9, targetHealth, accuracy);
		}
	}
	...
	function playNoteHitSound(note:Note):Void
	{
		...
		else if (hitsoundType != 'botplay' && !sustainSoundBool)
		{
			susHitSFXGroup.stop();
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.delayedDeath)
		{
			...
			else if (Conductor.bpm >= 150)
				sustainSoundBool = !sustainSoundBool;
			...
			if (!note.wasGoodHit)
			{
				...
				else
				{
					...
					if (allowHealthModifiers)
						targetHealth += PlayStateHelpers.calculateHealth(storyDifficulty, 8, targetHealth, accuracy);
					songScore += 5;
					maxSongScore += 5;
					vocals.volume = 1;
					if (mashPresses <= mashPressThreshold)
