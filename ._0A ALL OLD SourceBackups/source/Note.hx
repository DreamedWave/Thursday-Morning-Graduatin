...
class Note extends FlxSprite
{
	...
	public function fixPossibleGaps():Void
	{
		...
		if (prevNote != null && prevNote.isSustainNote && prevNote.active && active && !prevNote.finnaBeKilled && !prevNote.dunDidFixinsBool)
		{
	...
	override function update(elapsed:Float)
	{
		...
		if (mustPress)
		{
			...
			{
				if ((strumTime + (180 / scrollSpeed) < Conductor.songPosition - Conductor.safeZoneOffset * Conductor.timeScale && !isSustainTail)
					 || (strumTime + (5 / scrollSpeed) < Conductor.songPosition - Conductor.safeZoneOffset * Conductor.timeScale && isSustainTail))
				   tooLate = true;
			}
		...
		else
		{
			...
			if (strumTime <= Conductor.songPosition && strumTime > Conductor.songPosition - Conductor.safeZoneOffset * Conductor.timeScale)
				wasGoodHit = true;
				 ...
				 || (strumTime + 50 < Conductor.songPosition - Conductor.safeZoneOffset * Conductor.timeScale && forceMiss))
			   tooLate = true;
		}
	}
}
