...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		flashColor = (!Main.nightMode && !FlxG.save.data.nightmode ? flashColor : 0xFF8F8F7E);
	...
	function startSong():Void
	{
		...
		if (!FlxG.sound.music.playing) // don't restart the music if it's already playing
		{
			...
			if (SONG.song != "Finale")
			{
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 1, false, musicGroup);
	...
	{
		if (!endedSong && Conductor.songPosition < FlxG.sound.music.length - Conductor.crochet * 2)
		{
	...
	/*public*/ var sustainHeightMultiplier:Float = 1;
	var defaultScrollSpeed:Float = 1;
	/*public*/ var newScroll:Float = 1;
	var keeledOver:Bool = false;
	var styledOnEm:Bool = false;
	...
	override public function update(elapsed:Float)
	{
		if (!resultsPresent && !endedSong)
		{
			...
			if (PlayStateChangeables.scrollSpeed != newScroll)
			{
				...
				PlayStateChangeables.scrollSpeed = FlxMath.roundDecimal(FlxMath.lerp(newScroll, PlayStateChangeables.scrollSpeed == 1 ? defaultScrollSpeed : PlayStateChangeables.scrollSpeed, CoolUtil.boundTo(1 - (elapsed * 15), 0, 1)), 2);
		...
		{
			if (!endedSong && !inResults && !resultsPresent)
			{
				if (controls.STYLE && !styledOnEm && !styleTimer.active)
				{
					styledOnEm = true;
					styleTimer.start(Conductor.crochet * 1.75 / 1000, 
				...
				{
					styledOnEm = false;
					//Shit way rn, perhaps do someting with curDecimal Beat
					if (curStep % 2 == 0)
					{
						healthLerp += PlayStateHealthModifiers.ModifyHealth(storyDifficulty, 14, health, accuracy, styleTimer.elapsedTime);
						songScore += 75;
					}
					//Placeholder Anims
			...
			if (hurtVignette.alpha > 0)
				hurtVignette.alpha = FlxMath.lerp(0, hurtVignette.alpha, CoolUtil.boundTo(1 - (elapsed * (Conductor.bpm * 0.01)), 0, 1));
			...
			if (!paused)
			{
				...
				if (FlxG.save.data.lagCompensation && generatedMusic)
				{
					...
					if (compensatedViaLagSpike)
						compensationTimer = new FlxTimer().start(1, function(tmr:FlxTimer)
						{
	...
	function playNoteHitSound(note:Note):Void
	{
		if (!note.isSustainNote)
		{
			...
			if (hitsoundType != 'botplay' && !note.withinCompensation)
			{
				...
				{
					case 'sick':
						if (styleTimer.active)
						{
							styleTimer.active = false;
							styleTimer.cancel();
							if (controls.STYLE)
								styledOnEm = true;
						}
