...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		wackyXtraText.setFormat("Century Gothic", 32, FlxColor.fromRGB(255, 255, 255), CENTER);
	...
	function doSpecialWacky():Void
	{
		...
		switch (wackySwitch)
		{
			...
			case 'likeimposterfromamoger' | 'amongusjokes' | 'anentiregeneration':
				sus.volume = 0.7;
				sus.play();
			case 'haha' | 'shitsaggresively' | 'bwap':
				doCamZooming = false;
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			if (!transitioning)
				closebuttonTween = FlxTween.tween(closeButton,{alpha: 0.25}, 0.5, {ease: FlxEase.expoInOut, startDelay: 5});
	...
	function pressedAnything(pressedWhat:Int)
	{
		switch (pressedWhat)
		{
			...
			case 2:
				...
				closebuttonTween.cancel();
				closebuttonTween = FlxTween.tween(closeButton,{alpha: 1}, 0.15, {ease: FlxEase.sineInOut});
