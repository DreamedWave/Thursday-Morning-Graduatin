...
class PlayState extends MusicBeatState
{
	...
	private var hitSoundSick:FlxSound;
	private var hitSoundGood:FlxSound;
	private var hitSoundBad:FlxSound;
	private var hitSoundShit:FlxSound;
	...
	override public function update(elapsed:Float)
	{
		...
		if (startedCountdown)
		{
			if (health <= 0 || (FlxG.keys.justPressed.R && FlxG.save.data.resetButton))
			{
				...
				{
					FlxG.sound.play(Paths.sound('lowHP'), 1);
					trace("nope.");
				}
			}
		}
	...
	override function beatHit()
	{
		...
		{
			if (!resultsPresent && !inCutscene && songStarted)
				resyncVocals();
		}
				...
				case 'Finale'://ADAPTIVE MUSIC!! WOOOOO!!!
					...
					if (curBeat % 16 == 0)
					{
						...
						else if (dangerLevel >= 50 && enemyHealth > 30)
						{
							...
							taiko = FlxG.sound.play(Paths.soundRandom('finaleAdaptiveMus/taiko_', 3, 5));
							taiko.time = drums.time;
