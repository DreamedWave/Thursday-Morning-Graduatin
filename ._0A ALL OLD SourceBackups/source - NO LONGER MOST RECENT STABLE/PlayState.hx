...
class PlayState extends MusicBeatState
{
	...
	var camFollowSpeed:Float = 0; //The higher the number is, the faster the camera moves
	...
	override public function create()
	{
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (stageCheck)
			{
				...
				case 'theCity':
					...
					cityStreets = new FlxSprite(-120, 70);
		...
		subtitleText = new FlxText(healthBarBG.x + 80, healthBarBG.y - 120, -0);
		subtitleText.text = "";
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'retaliation':
					...
					if (isStoryMode && !playedCutscene && !FlxG.save.data.video)
					{
						FlxG.sound.playMusic(Paths.videoMusic('week2/midcutscene2-MUSIC'), 0.8);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, 0.96);
	...
	function fancySongUpdate():Void
	{
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if ((daNote.mustPress && daNote.tooLate && !PlayStateChangeables.useDownscroll || 
			daNote.mustPress && daNote.tooLate && PlayStateChangeables.useDownscroll) && daNote.mustPress)
			{
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (campaignAccuracy == 0)
					campaignAccuracy = accuracy;
				else
					campaignAccuracy -= accuracy - 100;
				...
				if (storyPlaylist.length <= 0)
				{
					...
					//Clearing of campaign accuracy and resseting of bools
					toggledPracticeMode = false;
				...
				case 'Desperation':
					switch (curBeat)
					{
						...
						case 168:
							changeSubtitles("Lord, please help us!", 0xFF5ce766);
						case 172:
							changeSubtitles("[changeSubtitles(%NoOneCanHearYou%, 0xFF5ce766);]", 0xFF5ce766);
						case 174:
							changeSubtitles("[%%%%%%%%]", 0xFF5ce766);
						case 175:
							changeSubtitles("", 0xFF5ce766);
						...
						case 180:
							changeSubtitles("(I'm able to repeat that)", 0xFF4adaff);
						case 184:
