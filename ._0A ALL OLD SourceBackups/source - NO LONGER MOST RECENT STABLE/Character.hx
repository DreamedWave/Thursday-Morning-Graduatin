	...
	public function new(x:Float, y:Float, ?character:String = "guy-default", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'guy-default':
				...
				animation.addByPrefix('idleHOLD', 'GUY idlehold', 12, false);
			...
			case 'selfsert-default':
				...
				animation.addByPrefix('deathConfirm', "DreamedWave Dead confirm", 24, false);
		...
		if (!debugMode)
		{
			switch (curCharacter)
			{
				case 'table-default' | 'table-theborder' | 'gf-christmas' | 'gf-car' | 'gf-pixel':
					if ((!animation.curAnim.name.startsWith("sing") || animation.curAnim.curFrame >= 6) && !PlayState.midsongCutscene)
					{
						danced = !danced;
