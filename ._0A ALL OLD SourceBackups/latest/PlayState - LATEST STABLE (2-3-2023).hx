...
class PlayState extends MusicBeatState
{
	...
	private var noteHitSound:FlxSound;
	...
	{	
		...
		if (!songsPlayedIn.contains(songLowercase))
		{
			...
			switch (SONG.song)
			{
				...
				case "Variegated Skylines":
					stageEffectsPath = Paths.image('theCityPortalOpen/effects/', 'week3');
					var leLoopCount:Int = 0;
					for (i in 0...9)
					{
						preloadList.set(stageEffectsPath + 'stageParticle_' + leLoopCount, 'image');
						leLoopCount++;
					}
				case "Finale":
		...
		if (!preloadCheck)
		{
			...
			var imagesToCache:Array<String> = ["bad", "good", "miss", "noteSplashes", "shit", "sick", "skipButton"];
	...
	function camShake(isEarthquake:Bool = false):Void
	{
		...
		if (isEarthquake)
			...
			camGame.shake(0.03, 0.25, camShakeFinish);
	}
	...
	private function createStageParticle(color:FlxColor = FlxColor.WHITE, nextColor:FlxColor = FlxColor.BLACK):Void
	{
		var particlesNum:Int = FlxG.random.int(5, 20);
		...
		for (j in 0...3)
		{
			...
			{
				var particle:PsychEngineFancyStageShit.FancyStageParticle = new PsychEngineFancyStageShit.FancyStageParticle(stageEffectsPath, 0, 9, -400 + width * i + FlxG.random.float(-width / 5, width / 5), cityStreets.y + 200 + (FlxG.random.float(0, 125) + j * 40), color, nextColor);
				...
				if (!PlayStateChangeables.Optimize)
				{
					switch (curSong)
					{
						...
						case 'Variegated Skylines':
							...
							else if (curBeat % 4 == 0 && stageParticles.visible)
								createStageParticle();
							switch(curBeat)
							{
								case 0 | 1:
