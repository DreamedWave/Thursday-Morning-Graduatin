...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		subtitleText = new FlxText(healthBarBG.x + 70, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), -0);
		...
		{
			case "Desperation":
				FlxG.sound.load(Paths.sound('Table_Crash'));
			case "Retaliation":
	...
	function changeSubtitles(text:String = "", color:FlxColor = 0xFFFFFFFF, borderColour:FlxColor = 0xFF000000, blendMode:String = "NORMAL", alpha:Float = 1, bgAlpha:Float = 0.45, font:String = 'Playtime With Hot Toddies'):Void
	{
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && startedCountdown && !cannotDie)
		{
			...
			{
				lowHealthSoundVolume = 1.05 - health;
					...
					if (daNote.isSustainNote)
					{
						...
						if (daNote.animation.curAnim.name.endsWith('end') && daNote.prevNote != null)
						{
								if (daNote.prevNote.isSustainNote && !daNote.prevNote.finnaBeKilled)
									daNote.y = daNote.prevNote.y + daNote.prevNote.height + 15;
								else
									daNote.y += daNote.height / 2 + (13 * sustainHeightMultiplier);
						}
						else
						{
						...
						if (!PlayStateChangeables.botPlay)
						{
							if (!daNote.mustPress || (daNote.sustainActive && holdArray[Math.floor(Math.abs(daNote.noteData))]))
							{
								...
								if (daNote.y + daNote.offset.y * daNote.scale.y <= center)
								{
									...
									swagRect.bottom = daNote.height - 30; //the 30 is here just to be safe
	...
	function endSong():Void
	{
		if (!resultsPresent)
		{
			...
			//if (!loadRep)
			if (!skippedShit && FlxG.save.data.scoreScreen && !isStoryMode)
	...
	private function popUpScore(daNote:Note = null):Void
	{
		...
		//{
			...
			//Note splash stuff
			if (!FlxG.save.data.notesplash && !PlayStateChangeables.botPlay) //!PlayStateChangeables.botPlay && [REMOVED FOR TEXTING, PUT THIS TF BACK OK?!]
			{
		...
		if (!resultsPresent && !PlayStateChangeables.Optimize)
		{
			switch (curSong)
			{
				...
				case 'Variegated Skylines':
					if (FlxG.save.data.flashing)
					{
						if (curBeat % 2 == 1 && curBeat > 96 && curBeat < 128)
							stageOverlay.animation.play('singleFlash');
					}
					switch(curBeat)
					{
