...
class PlayState extends MusicBeatState
{
	...
	var bopBottomBoppers:Bool = false;
	...
	override public function create()
	{
		...
		camINF = new FlxCamera();

		camHUD.bgColor.alpha = 0;
		camINF.bgColor.alpha = 0;
		...
		scoreTxt = new FlxText(healthBarBG.x - 35 + healthBar.width / 4, healthBarBG.y + (healthBarBG.height * 2.5 - 1), 0, "", 20);
		...
		if (FlxG.save.data.lagCompensation)
		{
			...
			lagCompIcon.y = FlxG.height - lagCompIcon.height * 1.1;
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'retaliation':
					...
					if (isStoryMode && !playedCutscene && !FlxG.save.data.video)
					{
						FlxG.sound.load(Paths.videoMusic('week2/midcutscene2-MUSIC'));
						camHUD.alpha = 0;
						loadingIcon.alpha = 0;
						blackScreen.alpha = 1;
						inCutscene = true;
	...
	function changeSubtitles(text:String = "", color:FlxColor = 0xFFFFFFFF, borderColour:FlxColor = 0xFF000000, blendMode:String = "NORMAL", alpha:Float = 1, font:String = 'Playtime With Hot Toddies'):Void
	{
		if (hasSubtitles && FlxG.save.data.subtitles && !endedSong)
		{
			...
			else
			{
				...
				{
					skipText.alpha = 0.75;
					subtitleText.y = subtitleText.y - skipText.height;
				}
		...
		startTimer = new FlxTimer().start(startTimerTime, function(tmr:FlxTimer)
		{
			if (!skipCountdown)
			{
				...
				switch (swagCounter)
				{
					...
					case 4:
						var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image("countdown/go" + altSuffix));
						go.y = 185;
						go.scrollFactor.set();
						go.cameras = [camHUD];
	...
	function startSong():Void
	{
		...
		{
			skipActive = true;
			skipText = new FlxText(healthBarBG.x, healthBarBG.y + -100, -0);
	...
	private function fadeOutGameplayHUD(delayMultiplier:Float):Void
	{
		...
		FlxTween.tween(kadeEngineWatermark, {alpha: 0}, 0.5 * delayMultiplier, {type: ONESHOT, ease: FlxEase.quadOut});
	...
	override public function update(elapsed:Float)
	{
		...
		else
		{
			...
			if (!paused)
			{
				...
				if (vocals.volume <= 0 && FlxG.save.data.lagCompensation && (vocals.time > FlxG.sound.music.time + 1.5 || vocals.time < FlxG.sound.music.time - 1.5))
				{
			...
			{		
				...
				switch (SONG.song)
				{
					case "Sudden Confrontation":
						...
						if (!daNote.forceMiss)
						{
							if (!daNote.isSustainNote && !daNote.isParent && enemyCanFail)
								enemyMissRandomizer = FlxG.random.bool(accuracy / 4);
							if (enemyMissRandomizer)
							{
								daNote.forceMiss = true;
				...
				if (!daNote.tooLate && !daNote.forceMiss)
				{
				...
				if (daNote.tooLate)
				{
					if (daNote.noteStyle == 'mine' || daNote.forceMiss)
					{
	...
	private function popUpScore(daNote:Note = null):Void
	{
		...
		//{
			...
			//Note splash stuff
			if (!FlxG.save.data.notesplash) //!PlayStateChangeables.botPlay && [REMOVED FOR TEXTING, PUT THIS TF BACK OK?!]
			{
	...
	function noteSplashVFX(daNote:Note, daRating:String):Void
	{
		...
		if (hitsoundType == 'chill')
		{
			...
			if (daNote.noteStyle == 'mine' && !daNote.withinCompensation)
			{
				add(sploosh);
	...
	{
		cityLightTimeDivider = 1000 + Conductor.bpm;
	...
	{	
		...
		if (camFollow.x != dad.getMidpoint().x + 150 + dadCamFollowOffsetX && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		{
			...
			if (luaModchart != null)
			{
				dadCamFollowOffsetX = luaModchart.getVar("followXOffset", "float");
				dadCamFollowOffsetY = luaModchart.getVar("followYOffset", "float");
			}
			#end
		...
		if (camFollow.x != boyfriend.getMidpoint().x - 100 + bfCamFollowOffsetX && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		{
			...
			if (luaModchart != null)
			{
				bfCamFollowOffsetX = luaModchart.getVar("followXOffset", "float");
				bfCamFollowOffsetY = luaModchart.getVar("followYOffset", "float");
			}
			#end
			...
			switch (curSong)
			{
					...
					switch (curBeat)
					{
						...
						case 529:
							if (isStoryMode)
							{
								...
								FlxTween.tween(camGame, {zoom: 1}, Conductor.crochet * 8 / 1000, {type: ONESHOT, ease: FlxEase.expoInOut});
				...
				case 'Disagreement':
					switch (curBeat)
					{
						...
						case 168:
							changeSubtitles("([Responding])", 0xffadeafa);
						case 176:
							changeSubtitles();
						case 179:
				...
				case 'Retaliation':
					...
					switch (curBeat)
					{
						...
						case 32:
							//trace("YOU BETTER BE CHANGED FROM " +  camFollowSpeed);
							camFollowSpeed = 0.04;
							changeSubtitles("How could one say", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
							//trace("TO " + camFollowSpeed);
						//case 157:
							//glitchyGlitchEffect.active = true;
						//case 158:
							//glitchyGlitchEffect.active = false;
						case 36:
							changeSubtitles("How could one say something so demeaning?", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 40:
							changeSubtitles("To someone who", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 44:
							changeSubtitles("To someone who did all of the cleaning", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 47:
							changeSubtitles("To someone who did all of the cleaning after", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");	
						case 48:
							changeSubtitles("something young and dumb", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 54:
							changeSubtitles("something young and dumb and", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 56:
							changeSubtitles("something young and dumb and oh", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 58:
							changeSubtitles("something young and dumb and oh so", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 60:
							changeSubtitles("something young and dumb and oh so fragile.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 64:
							changeSubtitles();
						case 96:
							changeSubtitles("All your mother wanted was to", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 100:
							changeSubtitles("All your mother wanted was to give us cheer.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 104:
							changeSubtitles("Nobody expected that we'd", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 108:
							changeSubtitles("Nobody expected that we'd end up here.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 111:
							changeSubtitles("Just", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 112:
							changeSubtitles("Just you and me and", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 116:
							changeSubtitles("Just you and me and her", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 118:
							changeSubtitles("10", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 120:
							changeSubtitles("10 miles", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 122:
							changeSubtitles("10 miles far", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 123:
							changeSubtitles("10 miles far away into the city.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 128:
						...
						case 155:
							camZooming = false;
							camFollowing = false;
							changeSubtitles("{Because of that I sometimes wish that I was never your son.}", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.9);
						case 158:
						...
						case 160:
							...
							if(FlxG.save.data.distractions)
							{
								FlxTween.tween(bottomBoppers, {alpha: 0.65, y: bottomBoppers.y - bottomBoppers.height / 1.2}, 0.5, {type: ONESHOT, ease: FlxEase.quadOut});
								FlxTween.tween(theBorderBG, {alpha: 0.3}, 0.5, {type: ONESHOT, ease: FlxEase.quadOut});
							}
							characterBlurEffect.alpha = 0.8;
							bopBottomBoppers = true;
						...
						case 223:
							...
							if(FlxG.save.data.distractions)
							{
								FlxTween.tween(bottomBoppers, {alpha: 0, y: 590}, 1, {type: ONESHOT, ease: FlxEase.quadIn});
								FlxTween.tween(theBorderBG, {alpha: 0.5}, 0.5, {type: ONESHOT, ease: FlxEase.quadOut});
							}
						case 224:
							//glitchyGlitchEffect.active = false;
							characterBlurEffect.alpha = 0.5;
							bopBottomBoppers = false;
							dadCamFollowOffsetY = -25;
							bfCamFollowOffsetY -= 20;
							changeSubtitles("You still are", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 227:
							changeSubtitles("You still are an ungrateful little sick brat", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 232:
							changeSubtitles("Maybe one day you'll", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 235:
							changeSubtitles("Maybe one day you'll realize your mistakes", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 239:
							changeSubtitles("Maybe one day you'll realize your mistakes so that", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 240:
							changeSubtitles("You'll drown in regret", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 246:
							changeSubtitles("You'll drown in regret once", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 248:
							changeSubtitles("You'll drown in regret once it's", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 250:
							changeSubtitles("You'll drown in regret once it's too", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 252:
							changeSubtitles("You'll drown in regret once it's too late", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 254:
							changeSubtitles("You'll drown in regret once it's too late to", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 256:
							changeSubtitles("You'll drown in regret once it's too late to blatt.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "VCR OSD Mono");
						case 258:
							changeSubtitles();
						case 288:
							changeSubtitles("Oftentimes he comes home drunk.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 292:
							changeSubtitles("Then gets pissed off when you flunk.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 296:
						...
						case 304:
							changeSubtitles("Can't run from the truth", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 310:
							changeSubtitles("Why", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 312:
							changeSubtitles("Why don't", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 314:
							changeSubtitles("Why don't you", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 315:
							changeSubtitles("Why don't you just give up your own", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 318:
							changeSubtitles("Why don't you just give up your own fleeting youth", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 320:
							changeSubtitles("I don't care for what you think.", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 324:
							changeSubtitles("You'll just bring me to my brink.", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 328:
							changeSubtitles("You're a coward.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 332:
							changeSubtitles("You're a coward. All your fears have gone empowered.", 0xFF04001A, 0xFFAA8888, SUBTRACT, 0.2, "Times New Roman");
						case 336:
							changeSubtitles("You will NEVER EVER", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 344:
							changeSubtitles("You will NEVER EVER care", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 346:
							changeSubtitles("You will NEVER EVER care for", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 347:
							changeSubtitles("You will NEVER EVER care for how your family'll fare.", 0xFF4adaff, FlxColor.BLACK, NORMAL, 0.5);
						case 351:
							camZooming = false;
							//glitchyGlitchEffect.active = true;
							dad.playAnim('midsongAnim', true);
						...
						case 354:
							...
							if(FlxG.save.data.distractions)
							{
								FlxTween.tween(bottomBoppers, {alpha: 0.8, y: bottomBoppers.y - bottomBoppers.height / 1.2}, 0.5, {type: ONESHOT, ease: FlxEase.quadOut});
								FlxTween.tween(theBorderBG, {alpha: 0.3}, 0.5, {type: ONESHOT, ease: FlxEase.quadOut});
							}
						case 416:
							characterBlurEffect.alpha = 0.5;
						...
						case 480:
							...
							{
								if(FlxG.save.data.distractions)
									FlxTween.tween(bottomBoppers, {alpha: -10, y: 690}, 4.1142, {type: ONESHOT, ease: FlxEase.quadIn});
				...
				case 'Playing With Fire':
					if (FlxG.save.data.flashing)
					{
						if (curBeat % 2 == 1 && curBeat > 276 && curBeat < 305)
			...
			case "border":
				if (FlxG.save.data.distractions)
				{
					if (curSong == "Retaliation")
					{
						...
						else if (!bopBottomBoppers && bottomBoppers.animation.curAnim.name != "idle")
