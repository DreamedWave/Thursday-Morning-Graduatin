...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(!FlxG.save.data.distractions)
		{
			...
			lowHPOverlay.scale.x *= 0.8;
			lowHPOverlay.scale.y *= 0.8;
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (stageCheck)
			{
				...
				case 'theBorder':
					...
					FlxG.sound.cache(Paths.sound('glitch_1'));
					FlxG.sound.cache(Paths.sound('glitch_2'));
		...
		//Inst, InstLowHP, Voc, and Miscs Preloading
		FlxG.sound.cache(Paths.inst(PlayState.SONG.song));
		if (SONG.needsVoices)
			FlxG.sound.cache(Paths.voices(PlayState.SONG.song));
		if (SONG.needsAdaptiveMus)
			FlxG.sound.cache(Paths.adaptiveMus(PlayState.SONG.song));
		...
		switch (SONG.song)
		{
			...
			default:
				if (SONG.needsMiscs)
					FlxG.sound.cache(Paths.miscs(PlayState.SONG.song));
		}

		//Le GameOver Music and SFX
		FlxG.sound.cache(Paths.sound("deathsound" + stageSuffix));
		FlxG.sound.cache(Paths.music("gameOver" + stageSuffix));
		FlxG.sound.cache(Paths.music("gameOver" + stageSuffix + "_End"));

		lowHPHeartBeat = FlxG.sound.cache(Paths.sound('lowHP'));
		var gameplaySFX:Array<String> = ["skipSongIntro", "intro3", "intro2", "intro1", "introGo", "missnote1", "missnote2", "missnote3", "enemyMiss1", "enemyMiss2", "enemyMiss3", "GitarooFail", "GitarooPress", "GitarooHover"]; //THANK YOU STILL BSOD#2524 FROM HAXE DISCORD SERVER!!!!
		...
		if (allowNoteHitSounds && FlxG.save.data.notesfx)
		{
			...
			{
				FlxG.sound.cache(Paths.sound('Note_Mine'));
				FlxG.sound.cache(Paths.sound('Note_Trigger'));
			}
		}
	...
	function changeSubtitles(text:String, color:FlxColor = 0xFFFFFFFF, borderColour:FlxColor = 0xFF000000, blendMode:String = "NORMAL", alpha:Float = 1, bgAlpha:Float = 0.45, font:String = 'Playtime With Hot Toddies', isBold:Bool = false, isItalic:Bool = false):Void
	{
		if (hasSubtitles && FlxG.save.data.subtitles && !endedSong)
		{
			if (text == "")
			{
				trace("CLEARED SUBTITLES (IN A SMART WAY!!)");
				clearSubtitles();
			}
	...
	{ // this actually handles press inputs
		...
		{
			noNotesToHit = false;
			trace("YES FUCKING NOTES");
			if (FlxG.save.data.strumline)
			{
				grpStrumLine.forEach(function(strumlineBG:FlxSprite)
				{
					FlxTween.tween(strumlineBG, {alpha: 0.45}, Conductor.crochet / 2000, {type: ONESHOT, ease: FlxEase.elasticInOut});
				});
			}
		...
		{
			noNotesToHit = true;
			trace("NO FUCKING NOTES");
			if (FlxG.save.data.strumline)
			{
				grpStrumLine.forEach(function(strumlineBG:FlxSprite)
				{
					FlxTween.tween(strumlineBG, {alpha: 0.2}, Conductor.crochet / 2000, {type: ONESHOT, ease: FlxEase.sineInOut});
				});
			}
		}
					...
					switch (curSong)
					{
						case "Desperation" | "Disagreement" | "Retaliation":
							if (!startedCountdown && songStarted)
								if (randomStageSound.exists && randomStageSound.playing)
	...
	function resyncVocals():Void
	{
		...
		//instLowHP.time = Conductor.songPosition;
		vocals.play();
		miscs.play();
		instLowHP.play();
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && startedCountdown)
		{
			...
			if (allowHeartBeatSounds && !cannotDie)
			{
				lowHPHeartBeat.volume = FlxMath.lerp(lowHealthEffectVolume, lowHPHeartBeat.volume, CoolUtil.boundTo(1 - (elapsed * 15), 0, 1));
				lowHPOverlay.alpha = FlxMath.lerp(0, lowHPHeartBeat.volume, CoolUtil.boundTo(1 - (elapsed * 7 * (Conductor.bpm * 0.01)), 0, 1));
			}
		...
		{
			camGame.zoom = FlxMath.lerp(defaultCamZoom, camGame.zoom, CoolUtil.boundTo(1 - (elapsed * 4 * (Conductor.bpm * 0.01)), 0, 1));
		...
		{
			if (!resultsPresent && !inResults && !inCutscene && !PlayStateChangeables.botPlay && !usedTimeTravel)
			{
				if (!cannotDie)
				{
					if (allowHealthModifiers)
	...
	private function popUpScore(daNote:Note = null):Void
	{
		...
		if (!PlayStateChangeables.botPlay)
		{
			...
			if (daRating != 'miss')
			{
				...
				currentTimingShown.x = rating.x + 120;
	...
	function sploshThisShitUp(daNote:Note, daRating:String):Void
	{
		if (allowHealthModifiers && !daNote.withinCompensation && (lagCompIcon.alpha < 0.1 || daRating != 'shit') && !daNote.isSustainNote)
		{
			...
			sploosh.setupNoteSplash(daNote.x, daNote.y, daNote.rating, daNote.noteStyle, daNote.noteData, daBabyArrowY);
			//x and y here idfk
	...
	function enemyMiss(noteDirection):Void
	{
		...
		if(FlxG.save.data.missSounds)
		{
			...
			missSound.volume = FlxG.random.float(0.1, 0.15);
		}
	...
	function loadFinaleMusic():Void
	{
		if (deaths <= 0 && !hasReset)
		{
			...
			for (i in soundNameArr) 
				FlxG.sound.cache(Paths.sound("finaleAdaptiveMus/" + i));
	...
	function loadNoteHitSound():Void
	{
		if (PlayStateChangeables.botPlay)
		{
			...
			FlxG.sound.cache(Paths.sound("Note_botplay"));
	...
	//(note:Note, resetMashViolation = true)
	{
		...
		if (!note.isSustainNote)
		{
			...
			else if (FlxG.save.data.notesplash && !PlayStateChangeables.botPlay)
			...
			switch (curSong)
			{
				...
				case "Retaliation":
					switch (curBeat)
					{
						...
						case 54:
							changeSubtitles("{something young and dumb and}", 0x00000000, 0xFFD39D9D, SUBTRACT, 0.35, 0, "VCR OSD Mono");
						case 56:
							changeSubtitles("{something young and dumb and oh}", 0x00000000, 0xFFD39D9D, SUBTRACT, 0.35, 0, "VCR OSD Mono");
						case 58:
							changeSubtitles("{something young and dumb and oh so}", 0x00000000, 0xFFD39D9D, SUBTRACT, 0.35, 0, "VCR OSD Mono");
						case 60:
						...
						case 235:
							changeSubtitles("{Maybe one day you'll realize your mistakes}", 0x00000000, 0xFFD39D9D, SUBTRACT, 0.35, 0, "VCR OSD Mono");
						case 239:
	...
	override function beatHit()
	{
					...
					if (health < 1 && !midsongCutscene && allowHeartBeatSounds && !resultsPresent && !inResults && !inCutscene && !cannotDie && allowHealthModifiers)
					{
						if (Conductor.bpm <= 250)
						{
							if (health >= 0.7 && curBeat % 4 == 0 || health > 0.5 && curBeat % 2 == 0 || health < 0.5)
							{
								...
								lowHPOverlay.alpha = 1;
								#if cpp
								@:privateAccess
								{
									lime.media.openal.AL.sourcef(lowHPHeartBeat._channel.__source.__backend.handle, lime.media.openal.AL.PITCH, FlxG.random.float(0.75, 1.25));
								}
								#end
		...
		if (!inResults && !resultsPresent && !PlayStateChangeables.Optimize)
		{
			switch (curSong)
			{
				case 'Mic Test':
					...
					switch (curBeat)
					{
						...
						case 179:
							...
							if (FlxG.save.data.strumline)
								grpStrumLine.forEach(function(leStrussy:FlxSprite)
								{
									FlxTween.tween(leStrussy, {alpha: 1}, Conductor.crochet / 1000, {type: ONESHOT, ease: FlxEase.quadIn});
								});
							gfSpeed = 1;
						case 180:
							camFollowSpeed = 0.04;
