...
class PlayState extends MusicBeatState
{
	...
	{	
		...
		scoreTxt = new FlxText(healthBarBG.x - 35 + healthBar.width / 4, healthBarBG.y + 35, 0, "", 20);
		...
		if (!songsPlayedIn.contains(songLowercase))
		{
			...
			switch (SONG.song)
			{
				...
				case "Finale":
					loadFinaleMusic();
				default:
					if (SONG.needsMiscs && !FlxG.save.data.cacheMusic)
			...
			if (dadSwaperoo != "" && !FlxG.save.data.cacheImages)
				imagesToCache.push("characters/" + dadSwaperoo + ".png");
		}
		#if debug
		else
		...
		{
			#if debug
			trace('what i am about to do    has not been approved   by the Vatican');
			#end
			popUpScore();
			...
			imagesToCache = ["bad.png", "good.png", "miss.png", "noteSplashes.png", "shit.png", "sick.png", "skipButton.png"];
		...
		{
			var imageNames = i.replace(".png","");
			var data:BitmapData = BitmapData.fromFile("assets/shared/images/" + i);
			#if debug
			trace('id ' + imageNames + ' file - assets/shared/images/' + i);
			#end
			var cachedImages = FlxGraphic.fromBitmapData(data);
			daImageShitData.set(imageNames, cachedImages);
		}
	}
			...
			if (!skipCountdown)
			{
				if (!PlayStateChangeables.Optimize)
				{
					switch(songLowercase)
					{
						...
						case "disagreement" | "playing-with-fire":
							dad.dance();
							gf.dance();
	...
	override public function update(elapsed:Float)
	{
		if (!resultsPresent && !endedSong)
		{
			...
			if (PlayStateChangeables.scrollSpeed != newScroll)
			{
				...
				PlayStateChangeables.scrollSpeed = FlxMath.lerp(newScroll, PlayStateChangeables.scrollSpeed, CoolUtil.boundTo(1 - (elapsed * 15), 0, 1));
	...
	function camShake(isEarthquake:Bool = false):Void
	{
		...
		if (isEarthquake)
			camGame.shake(0.035, 0.2, camShakeFinish);
		else
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		...
		if (!boyfriend.stunned && allowHealthModifiers && !daNote.withinCompensation)
		{
			...
			//NOTE MISS SOUND
			if(FlxG.save.data.missSounds && daNote.noteStyle != 'mine' && daNote.noteStyle != 'trigger')
			{
	...
	function loadNoteHitSounds():Void
	{
		if (PlayStateChangeables.botPlay)
		{
	...
	override function beatHit()
	{
		...
		if (beatCheck < curBeat)
		{
			...
			{
				case "border":
					if (FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset && FlxG.save.data.distractions && !midsongCutscene && !resultsPresent && !boyfriend.animation.curAnim.name.startsWith('sing'))
						lightningStrikeShit();
