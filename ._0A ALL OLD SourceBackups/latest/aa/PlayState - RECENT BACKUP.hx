...
class PlayState extends MusicBeatState
{
	...
	function startSong():Void
	{
		...
		else if (!FlxG.sound.music.playing) // don't restart the music if it's already playing
		{
			//trace("reestarded");
			startedMusic = true;
			if (!paused)
			...
			{		
				...
				switch (SONG.song)
				{
					case "Sudden Confrontation":
						...
						if (enemyCanFail && enemyMissRandomizer && SONG.notes[Math.floor(curStep / 16)] != null && !SONG.notes[Math.floor(curStep / 16)].altAnim)
				...
				if (!enemyMissRandomizer && (!dad.animation.curAnim.name.startsWith("sing") || dad.animation.curAnim.curFrame >= 1))
					dad.playAnim('sing' + dataSuffix[singData] + altAnim, true);

				if (FlxG.save.data.cpuStrums && !enemyMissRandomizer)
				{
					cpuStrums.forEach(function(spr:FlxSprite)
					{
				...
				if (!enemyMissRandomizer || daNote.tooLate)
				{
					daNote.kill();
					notes.remove(daNote, true);
					daNote.destroy();
					//trace('deleted nenemy note');
				}
			}
