...
class Paths
{
	...
	inline static public function videoMusic(key:String, ?library:String)
	{
		trace('assets/videos/$key.$SOUND_EXT');
		return getPath('videos/$key.$SOUND_EXT', MUSIC, library);
	}
	...
	inline static public function miscs(song:String)
	{
		var songLowercase = StringTools.replace(song, " ", "-").toLowerCase();
		return 'songs:assets/songs/${songLowercase}/Miscs.$SOUND_EXT';
	}

	inline static public function adaptiveMus(song:String)
	{
		var songLowercase = StringTools.replace(song, " ", "-").toLowerCase();
