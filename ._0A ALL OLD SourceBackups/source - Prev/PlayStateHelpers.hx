...
class HealthModifiers
{
    public static function modifyHealth(difficulty:Int, hpDrainType:Int, health:Float, accuracy:Float, externalValue:Float = 0)
    {
        ...
        switch (hpDrainType)
        {
            ...
            case 14:
                #if debug
                trace (externalValue + " (MS THINGY)");
                #end
                var mult = (45 - externalValue) / 45;
                if (mult < 0)
                    mult = 0;
                if (health < 2)
                  feedbackHealth = ((2 - health) / 4) * mult;
        }
...
class LerpTime
{
    public static function calculate(timeElapsed:Float, durationMultiplier:Float, min:Int = 0, max:Int = 0)
    {
...
/*class Lerper
{
    public static function lerpGivenNumbers(start:Float, end:Float, timeElapsed:Float, durationMultiplier:Float, min:Int = 0, max:Int = 1, roundReturnedValue:Bool = false, decimalCount:Int = 5)
    {
        var returnValue:Float = 0;
        returnValue = FlxMath.lerp(end, start, CoolUtil.boundTo(1 - (timeElapsed * durationMultiplier), min, max));
        if (roundReturnedValue)
            returnValue = FlxMath.roundDecimal(returnValue, decimalCount);
        return returnValue;
    }
}*/