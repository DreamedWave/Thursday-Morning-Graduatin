...
class TitleState extends MusicBeatState
{
	...
	function doSpecialWacky():Void
	{
		...
		switch (wackySwitch)
		{
			...
			case 'idunno':
				wackyXtraText.text = '(oh actually today is a ' + Caching.curDay + "!!)";
				wackyXtraText.visible = true;
				FlxG.log.add('WackyText');
			case 'asunday' | 'amonday' | 'wereturntoclass':
				if (Caching.curDay == 'Sunday')
				...
				FlxG.log.add('WackyText');
			case 'wasnow1am':
				if(Caching.curHour == "1 AM")
					wackyXtraText.text = '(you should go to sleep-)';
				else
					wackyXtraText.text = '(bro it is literally ' + Caching.curHour + '-)';
				wackyXtraText.visible = true;
				FlxG.log.add('WackyText');
		...
		switch (pressedWhat)
		{
			case 1:
				if (!transitioning && !failSafeAugh)
				{
					...
					//if (Caching.curDay == 'Thursday') doAchienvementshit()
			...
			case 2:
				...
				{
					closebuttonTween = FlxTween.tween(closeButton,{alpha: 1}, 0.15, {type: ONESHOT, ease: FlxEase.cubeOut});
					if (FlxG.save.data.camzoom)
					{
						FlxG.camera.zoom -= 0.01;
					}
