...
class PlayState extends MusicBeatState
{
	...
	//idfk gawd
	var characterWiggleEffect:WiggleEffect;
	var updateCharacterWiggle:Bool = false;
	...
	private var camTweened:Bool;

	private var idleCamShakeTimer:Float = 0;

	var grpRatings = new FlxTypedGroup<FlxSprite>();
	...
	override public function create():Void
	{
		...
		if (isStoryMode && deaths == 0 && !hasReset)
		{
					...
					if (SONG.player2 == "priest-theborderangry")
					{
						if (FlxG.save.data.distractions)
						{
							if (!PlayStateChangeables.Optimize)
							{
								...
								characterBlurEffect.alpha = 0.65;
								add(characterBlurEffect);

								updateCharacterWiggle = true;
								characterWiggleEffect = new WiggleEffect();
								characterWiggleEffect.effectType = WiggleEffectType.DREAMY;
								characterWiggleEffect.waveAmplitude = 0.0015;
								characterWiggleEffect.waveFrequency = 100;
								characterWiggleEffect.waveSpeed = 4;
								dad.shader = characterWiggleEffect.shader;
		...
		scoreTxt = new FlxText(healthBarBG.x - 35 + healthBar.width / 4, healthBarBG.y + 40, 0, Ratings.CalculateRanking(songScore, songScoreDef, nps, maxNPS, accuracy), 20);
		...
		switch (songLowercase)
		{
			case 'mic-test':
				...
				tutorialText = new FlxTypedGroup<FlxText>();
				add(tutorialText);
		...
		else
		{
			switch (songLowercase)
			{
				case 'mic-test' | 'desperation' | 'disagreement' | 'retaliation' | 'kid-with-a-gun' | 'playing-with-fire' | 'variegated-skylines':
					hasSubtitles = true;
			}
			startCountdown();
			camZooming = true;
		...
		//trace ('defaultMashingLimit = ' + defaultMashingThreshold);
		...
		new FlxTimer().start(Conductor.crochet / 1000, function(swagTimer:FlxTimer)
		{
			...
			new FlxTimer().start(Conductor.crochet / 1000, function(swagTimer:FlxTimer)
			{
				...
				new FlxTimer().start(Conductor.crochet / 1000, function(swagTimer:FlxTimer)
				{
					...
					{
						camShake(true, 'camGame', 0.04, 0.45);
			...
			if (!skipCountdown)
			{
				if (!PlayStateChangeables.Optimize)
				{
					switch(songLowercase)
					{
						...
						case "variegated-skylines":
							camShake(false, 'camGame', 0.002, idleCamShakeTimer);
	...
	function handleInput(event:KeyboardEvent):Void // this actually handles press inputs
	{
		...
		if (startedCountdown && key > -1 && FlxG.keys.checkStatus(eventKey, JUST_PRESSED))
		{
			...
			if (mashPresses > mashPressThreshold && isBFTurn)
			{
				accuracy -= 0.15;
				targetHealth -= 0.0255;
				updateAccuracy();
				//trace("BRO STOP SPAMMING - " + mashPresses);
			}
			...
			if (sortedNotesList.length > 0) 
			{
				for (epicNote in sortedNotesList)
				{
					...
					if (!notesStopped) 
					{
						...
						if (!isStoryMode) // hummuna humman change this if you want to reenable results in story mode (6)
						{
							ana.hit = true;
	...
	private function startSong():Void
	{
		...
		#if debug
		trace('1 Beat before music end = ' + musicTimeCusp);
		#end
	...
	var styleTimer:FlxTimer = new FlxTimer();
	var isntSpammingStyle:Bool = false;
	var allowStyleBonus:Bool = true;
	...
	function styleOnEm(ms:Float):Void
	{
		...
		//Style bonus
		allowStyleBonus = true;

		if (allowStyleBonus && stylePresses <= 5)
		{
			allowStyleBonus = false;
			var styleTime:Float = (Conductor.crochet / 1000) - ms * 0.01;
			styleTimer.start(styleTime, function(tmr:FlxTimer)
			{
				allowStyleBonus = true;
			});
			switch (styleRating)
			{
				case 'sick' | 'good':
					if (styleRating == 'sick' || isBFTurn)
					{
						targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 14, health, accuracy, ms);
						songScore += 75;
						if (storyWeek < 3)
						{
							if (storyWeek > 0)
	...
	override public function update(elapsed:Float)
	{
		if (!resultsPresent && !endedSong)
		{
			...
			if (PlayStateChangeables.scrollSpeed != newScroll)
			{
				...
				PlayStateChangeables.scrollSpeed = FlxMath.roundDecimal(FlxMath.lerp(newScroll, PlayStateChangeables.scrollSpeed == 1 ? defaultScrollSpeed : PlayStateChangeables.scrollSpeed, PlayStateHelpers.LerpTime.calculate(elapsed, 15, 0, 1)), 2);
		...
		if (generatedMusic && !paused && !endedSong)
		{
		...
		if (generatedMusic && startedCountdown)
		{
			...
			{
				if (controls.STYLE && generatedMusic && boyfriend.animation.curAnim.name.startsWith('style') && boyfriend.animation.curAnim.curFrame >= 3)
				{
			...
			{
				case 'border':
					if (updateCharacterWiggle)
						characterWiggleEffect.update(elapsed);
				case 'cityskyline':
			...
			if (hurtVignette.alpha > 0)
				hurtVignette.alpha = FlxMath.lerp(0, hurtVignette.alpha, PlayStateHelpers.LerpTime.calculate(elapsed, (Conductor.bpm * 0.01), 0, 1));

			lowHealthEffectVolume = 1 - health;
			lowHPHeartBeat.volume = FlxMath.lerp(lowHealthEffectVolume, lowHPHeartBeat.volume, PlayStateHelpers.LerpTime.calculate(elapsed, 15, 0, 1));
			
			if (lowHPOverlay.alpha > 0)
			{
				if (FlxG.save.data.flashing)
					lowHPOverlay.alpha = FlxMath.lerp(0, lowHPOverlay.alpha, PlayStateHelpers.LerpTime.calculate(elapsed, 4.5 * (Conductor.bpm * 0.01), 0, 1));
				else
					lowHPOverlay.alpha = FlxMath.lerp(lowHPHeartBeat.volume, lowHPOverlay.alpha, PlayStateHelpers.LerpTime.calculate(elapsed, 6 * (Conductor.bpm * 0.01), 0, 1));
			}
			...
			if (SONG.needsAdaptiveMus)
			{
				...
				if (health < 1)
				{
					FlxG.sound.music.volume = FlxMath.lerp(health - 0.3, FlxG.sound.music.volume, PlayStateHelpers.LerpTime.calculate(elapsed, 15, 0, 1));
					instLowHP.volume = FlxMath.lerp(lowHealthEffectVolume + 0.3, instLowHP.volume, PlayStateHelpers.LerpTime.calculate(elapsed, 15, 0, 1));
					//instLowHP.time = FlxG.sound.music.time;
				}
			...
			scoreTxt.screenCenter(X); //NVM BRO LITERALLY JUST SCREENCENTER IT DUMBASS LMAO
		}

		if (controls.PAUSE && startedCountdown && canPause && !paused && FlxG.sound.music.time < musicTimeCusp)
			pauseGame();
		...
		if (FlxG.save.data.distractions)
		{
			var mult1:Float = FlxMath.lerp(1, iconP1.scale.x, PlayStateHelpers.LerpTime.calculate(elapsed, 7.5 * (Conductor.bpm * 0.01), 0, 1));
			iconP1.scale.set(mult1, mult1);
			iconP1.updateHitbox();
	
			var mult2:Float = FlxMath.lerp(1, iconP2.scale.x, PlayStateHelpers.LerpTime.calculate(elapsed, 7.5 * (Conductor.bpm * 0.01), 0, 1));
		...
		if (FlxG.keys.justPressed.SPACE && skipActive && generatedMusic && startedCountdown && curBeat >= 0)
		{
			...
			if (songLowercase == 'mic-test')
			{
				...
				for (i in 0...4)
				{
					if (tutorialText.members.length > 0)
		...
		else
		{
			...
			//Health lerp shits
			health = FlxMath.lerp(targetHealth, health, PlayStateHelpers.LerpTime.calculate(elapsed, 19, 0, 1));
			Conductor.songPosition += FlxG.elapsed * 1000;
			...
			if (!paused)
			{
				...
				if (FlxG.save.data.lagCompensation && generatedMusic)
				{
					...
					//this might be shit but idk
					//gonna try not including this

					//LagSpike Compensation Attempt 2
					//if (openfl.Lib.current.stage.frameRate < lagspikeDetectionThreshold)
					...
					if (compensatedViaLagSpike)
					{
						...
						{
							//trace ('reset compensation timer');
							compensationTimerResetCheck++;
							compensationTimer.reset(compensationTime);
						}
						else
						{
							//trace ('stopped compensation timer cuz its too much ok,,,');
		...
		{
			camGame.zoom = FlxMath.lerp(defaultCamZoom, camGame.zoom, PlayStateHelpers.LerpTime.calculate(elapsed, 2.5 * (Conductor.bpm * 0.01), 0, 1));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, PlayStateHelpers.LerpTime.calculate(elapsed, 3 * (Conductor.bpm * 0.01), 0, 1));
		}
		...
		if (startedCountdown && (targetHealth <= 0 || (FlxG.keys.justPressed.R && FlxG.save.data.resetButton)))
		{
	...
	function camShake(force:Bool = true, camToShake:String = 'camGame', intensity:Float = 0.03, duration:Float = 0.25):Void
	{
		switch (camToShake)
		{
			case 'camGame':
				camGame.shake(intensity, duration, force);
			case 'camHUD':
				camHUD.shake(intensity, duration, force);
		}
	}
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if (PlayStateChangeables.useDownscroll)
			{
				...
				if (daNote.isSustainNote)
				{
					...
					{
						daNote.y += 10.5 * (fakeCrochet / 400) * 1.5 * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed, 2) + (46 * (FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed, 2) - 1));
						daNote.y -= 46 * (1 - (fakeCrochet / 600)) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed, 2);
						if (!daNote.prevNote.isParent || daNote.prevNote == null)
							daNote.y -= 25 / sustainHeightMultiplier;
					}
					daNote.y += (Note.swagWidth / 2) - (60.5 * (FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed, 2) - 1));
					daNote.y += 27.5 * ((SONG.bpm / 100) - 1) * (FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed, 2) - 1);

					//FIX THIS SHIT!!!
					// If not in botplay, only clip sustain notes when properly hit, botplay gets to clip it everytime
					//Im yOINKING THE DOWNSCROLL SHIT FROM PSYCH-
					if (!daNote.mustPress || ((daNote.sustainActive && holdArray[Math.floor(Math.abs(daNote.noteData))]) || PlayStateChangeables.botPlay))
					{
			...
			else
			{
				...
				if (daNote.mustPress)
					daNote.y = (playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y 
						- 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed, 2));
				else
					daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y 
						- 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed, 2));
				...
				if (daNote.isSustainNote)
				{
					...
					//Moved to the update function of Note.hx
					if (daNote.isSustainTail && daNote.prevNote != null)
					{
						if (daNote.prevNote.isSustainNote && !daNote.prevNote.finnaBeKilled)
							daNote.y = daNote.prevNote.y + daNote.prevNote.height - (20 / sustainHeightMultiplier);
						else
							daNote.y -= daNote.height / 2 + (25 / sustainHeightMultiplier);
					}
					else
						daNote.y -= daNote.height / 2 - (daNote.childBehindParent ? 20 : 0);
					...
					if (!daNote.mustPress || ((daNote.sustainActive && holdArray[Math.floor(Math.abs(daNote.noteData))]) || PlayStateChangeables.botPlay))
					{
						...
						if (daNote.y + daNote.offset.y * daNote.scale.y <= center)
						{
							...
							swagRect.bottom = (swagRect.y + (daNote.height * daNote.scale.y + 30)) * sustainHeightMultiplier * Math.sqrt(PlayStateChangeables.scrollSpeed); //basically playing it safe
								...
								default:
									if (allowHealthModifiers && !daNote.withinCompensation && hurtVignette.alpha < 0.2)
									{
										if (daNote.isParent)
										{
											...
											targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 1, targetHealth, accuracy);
											totalNotesHit -= 1;
										...
										else
										{
											...
											targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 0, targetHealth, accuracy);
											vocals.volume = 0;
										}
										noteMiss(daNote.noteData, daNote);
									}
						...
						else
						{
							if (!daNote.isParent && !daNote.wasGoodHit && allowHealthModifiers && !daNote.withinCompensation && hurtVignette.alpha < 0.2)
							{
								if (daNote.sustainActive && daNote.spotInLine != daNote.parent.children.length)
								{
									...
									targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 2, targetHealth, accuracy);
									...
									targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 3, targetHealth, accuracy);
							}
						}
					}
						...
						if (!daNote.isSustainNote)
						{
							switch (daNote.noteType)
							{
								...
								case 'trigger':
									...
									if (allowHealthModifiers && !daNote.withinCompensation && hurtVignette.alpha < 0.2)
									{
										FlxG.sound.play(Paths.sound('Note_Mine'), 1, false, hitSFXGroup);
										boyfriend.playAnim('hurt', true);
										camShake(true, 'camGame', 0.05);
										camShake(true, 'camHUD', 0.005);
									}
							}
						}
					}
	...
	function getFuckingShot():Void
	{
		...
		if (allowHealthModifiers)
		{
			...
			if (timesShot <= 2)
			{
				if (targetHealth < 0.7 && timesShot <= 2)
					targetHealth = 0.25;
				else
					targetHealth = 0.05;
			}
	...
	var nextStateIsPlayState:Bool = false;
		...
		if (allowHealthModifiers && !daNote.withinCompensation)
		{
			switch (daRating)
			{
				...
				case 'shit':
					if (lagCompIcon.alpha < 0.1)
					{
						...
						if (hurtVignette.alpha < 0.2)
							targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 4, targetHealth, accuracy);
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit -= 1;
						timingColour = FlxColor.RED;
					}
				case 'bad':
					...
					//Health Drain
					if (hurtVignette.alpha < 0.2)
						targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 5, targetHealth, accuracy);
					ss = false;
					bads++;
					if (FlxG.save.data.accuracyMod == 0)
				...
				case 'good':
					daRating = 'good';
					//Health Drain/Gain
					targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 6, targetHealth, accuracy);
					score = 200;
					ss = false;
					goods++;
					if (FlxG.save.data.accuracyMod == 0)
				...
				case 'sick':
					//Health Gain
					targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 7, targetHealth, accuracy, hurtVignette.alpha);
					if (FlxG.save.data.accuracyMod == 0)
						if (mashPresses <= mashPressThreshold)
							totalNotesHit += 1;
					sicks++;
		...
		if (!PlayStateChangeables.botPlay)
		{
			...
			if (daRating != 'miss')
			{
				...
				if (currentTimingShown != null)
					grpRatings.remove(currentTimingShown);
				...
				if (currentTimingShown.exists)
					grpRatings.add(currentTimingShown);
			}
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			//if (!loadRep)
			for (i in anas)
				if (i != null && !isStoryMode) // hummuna humman change this if you want to reenable results in story mode (5)
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		...
		{
			bypassOppMissCheck++;
			if (bypassOppMissCheck > 15 / storyDifficulty)
				bypassOppMissGain = true;
	...
	var bypassOppMissGain:Bool = false;
	var bypassOppMissCheck:Int = 0;

	private function enemyMiss(noteDirection):Void
	{
		...
		if (healthBar.percent < 85 && !bypassOppMissGain)
			targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 9, targetHealth, accuracy);
	}
	...
	function playNoteHitSound(note:Note):Void
	{
		if (!note.isSustainNote)
		{
			...
			FlxG.sound.play(Paths.sound('Note_' + hitsoundType + '_Sustain'), noteHitVolume, false, hitSFXGroup);
	}
		...
		if (!note.tooLate && !note.forceMiss)
		{
			...
			switch (SONG.song)
			{
				case "Sudden Confrontation" | "Sprouting Irritation":
					...
					{
						if (!note.isSustainNote)
							targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 10, targetHealth, accuracy);
						else
							targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 11, targetHealth, accuracy);
					}
			}
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			else
			{
				...
				if (allowHealthModifiers)
					targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 8, targetHealth, accuracy);
				vocals.volume = 1;
				if (mashPresses <= mashPressThreshold)
					totalNotesHit += 1;
			}
			...
			switch (note.noteType)
			{
				...
				case 'trigger':
					...
					dad.playAnim('gunSHOOT', true);
					camShake(true);
					FlxG.sound.play(Paths.sound('Note_Trigger'), 0.75, false, hitSFXGroup);
					sicks++;
					//la health gain for trigger notes
					targetHealth += PlayStateHelpers.HealthModifiers.modifyHealth(storyDifficulty, 12, targetHealth, accuracy);
	...
	{
		stageSound = FlxG.sound.play(Paths.soundRandom('glitch_', 1, 2, 'week2'));
	...
	{	
		...
		if (PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (mashPresses != 0 && mashPressThreshold != defaultMashingThreshold)
			{
				mashPressThreshold = defaultMashingThreshold;
				mashPresses = 0;
			}
			...
			if (doCamFollowing && !camGame.isShakingCam)
			{
			...
			switch (curSong)
			{
				...
				case "Retaliation":
					switch (curBeat)
					{
						...
						case 256:
							changeSubtitles("{You'll drown in regret once it's too late for change.}", 0x00000000, 0xFFD39D9D, SUBTRACT, 0.35, 0, "VCR OSD Mono");
						case 258:
							clearSubtitles();
						case 288:
		...
		{			
			...
			if (!resultsPresent && !inResults)
			{
				switch (curSong)
				{
					...
					default:
						if (FlxG.save.data.camzoom && camZooming && curBeat % 4 == 0)
						{
							camGame.zoom += 0.025;
							camHUD.zoom += 0.015;
						}
			...
			if (!inResults && !resultsPresent)
			{
				...
				if (!PlayStateChangeables.Optimize && FlxG.save.data.distractions)
				{
					switch (curSong)
					{
						...
						case 'Retaliation':	
							if (inSongClimax)
								camShake(false, 'camGame', 0.003, idleCamShakeTimer);
							...
							switch (curBeat)
							{
								...
								case 160:
									...
									characterWiggleEffect.waveSpeed = 8;
									characterWiggleEffect.waveAmplitude = 0.002;
									characterBlurEffect.blend = ADD;
									characterBlurEffect.color = 0xff8080;
								...
								case 223:
									...
									characterWiggleEffect.waveAmplitude = 0.0015;
									characterBlurEffect.alpha = 0.65;
									characterBlurEffect.blend = NORMAL;
									characterBlurEffect.color = 0xffffff;
									inSongClimax = false;
								case 224:
									//glitchyGlitchEffect.active = false;
									defaultCamZoom = 0.95;
								...
								case 288:
									defaultCamZoom = 0.99;
									characterWiggleEffect.waveSpeed = 8;
									characterWiggleEffect.waveAmplitude = 0.001;
									characterBlurEffect.blend = SUBTRACT;
									characterBlurEffect.alpha = 0.3;
									characterBlurEffect.color = 0xFF5ce766;
								case 347:
									camZooming = false;
									camGame.zoom -= 0.04;
								case 351:
									characterWiggleEffect.waveSpeed = 4;
									characterWiggleEffect.waveAmplitude = 0.0015;
									characterBlurEffect.color = 0xac5050;
									characterBlurEffect.alpha = 0.65;
									//glitchyGlitchEffect.active = true;
									dad.playAnim('midsongAnim', true);
								case 352:
								...
								case 416:
									...
									characterWiggleEffect.waveSpeed = 6;
									characterBlurEffect.blend = NORMAL;
									characterWiggleEffect.waveAmplitude = 0.001;
									defaultCamZoom = 0.82;
									dadCamFollowOffsetY = -25;
								...
								case 479:
									...
									camZooming = false;
								case 480:
									characterWiggleEffect.waveSpeed = 2;
									inSongClimax = false;
									bfCamFollowOffsetY -= 10;
								...
								case 508:
									...
									camShake(true, 'camGame', 0.003, idleCamShakeTimer - 0.05);
									FlxTween.tween(camGame, {zoom: 1.65}, 2, {type: ONESHOT, ease: FlxEase.quadIn});
								case 509:
									//glitchyGlitchEffect.active = false;
									boyfriend.playAnim('concerned', true);
									dad.playAnim('endsongAnim1', true);
									camShake(true, 'camGame', 0.008, idleCamShakeTimer - 0.05);
								case 510:
									dad.playAnim('endsongAnim2', true);
									camShake(true, 'camGame', 0.01, idleCamShakeTimer);
								case 511:
									characterWiggleEffect.waveSpeed = 10;
									boyfriend.playAnim('scared', true);
									dad.playAnim('endsongAnim3', true);
									camShake(true, 'camGame', 0.045, Conductor.crochet / 1000);
									camShake(true, 'camHUD', 0.005, Conductor.crochet / 1000);
									theBorderBG.animation.play('lightning');
								case 512:
									remove(boyfriend);
									remove(dad);
						...
						case 'Variegated Skylines':
							...
							{
								if (!inSongClimax)
									camShake(false, 'camGame', 0.0025, idleCamShakeTimer);
								else
									camShake(false, 'camGame', 0.004, idleCamShakeTimer);
								if (curBeat % 2 == 0)
									createStageParticle('theCityPortalOpen/effects/stageParticle');
							}
							...
							switch(curBeat)
							{
								...
								case 447:
									...
									defaultCamZoom = 0.85;
								case 448:
									camShake(true, 'camGame', 0.035, 0.3);
							}
