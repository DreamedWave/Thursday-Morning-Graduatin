...
class FunnySplash extends MusicBeatState
{
	var sprite:Sprite;
	var gfx:Graphics;
	var text:TextField;
	...
	override public function create():Void
	{
		...
		var dtf = new TextFormat('VCR OSD Mono', 16, 0xffffff);
		...
		lilOpeningAnim.animation.addByPrefix('danec', "lilDance", 20, true);
		...
		FlxG.sound.playMusic(Paths.music('splashScreenJingle'), 1, false);
	...
	override function update(elapsed)
	{
		...
		if (FlxG.keys.justPressed.ANY && !FlxG.keys.justPressed.ZERO && !FlxG.keys.justPressed.PLUS && !FlxG.keys.justPressed.MINUS && !wentForTheMilk)
		{
			wentForTheMilk = true;
			finishSplash();
		}
