...
class PlayState extends MusicBeatState
{
	...
	var adaptiveMusicGroup1:FlxTypedGroup<FlxSound> = null;
	var adaptiveMusicGroup2:FlxTypedGroup<FlxSound> = null;
	var adaptiveMusicGroup3:FlxTypedGroup<FlxSound> = null;
	var adaptiveMusicGroup4:FlxTypedGroup<FlxSound> = null;
	var adaptiveMusicGroup5:FlxTypedGroup<FlxSound> = null;
	...
	private var camFollowSpeed:Float = 0.09;
	//The higher the number is, the slower the camera moves
	...
	public var midsongCutscene:Bool = false;
	var noteHitSounds:FlxTypedGroup<FlxSound> = null;
	...
	override public function create()
	{
		...
		hurtVignette.scale.x *= 1;
		hurtVignette.scale.y *= 1;
		...
		switch (SONG.player2)
		{
			...
			case "priest-theborderangry":
				...
				if (FlxG.save.data.distractions)
				{
					if (!PlayStateChangeables.Optimize)
					{
						var motionTrail = new FlxTrail(dad, null, 3, 8, 0.2, 0.08);
						add(motionTrail);
					}
				}
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (stageCheck)
			{
				...
				case 'theCity':
					...
					//                                 X    Y
					var bg:FlxSprite = new FlxSprite(-140, -50).loadGraphic(Paths.image('philly/sky', 'week3'));
					...
					streetBehind.scrollFactor.set(0.7, 0.7);
					...
					cityStreets = new FlxSprite(-120, 40);
					cityStreets.frames = streetTex;
					cityStreets.scrollFactor.set(0.95, 0.95);
					...
					vignette = new FlxSprite(-250, -130);
					vignette.frames = vignetteTex;
		...
		avoidBulletTipGraphic = new FlxSprite().loadGraphic(Paths.image('infocards/TutorialTipPlaceHolder', 'tutorial'));
		...
		//Song Start Events
		if (!hasReset && !PlayStateChangeables.Optimize)
		{
		...
		{
			noteHitSounds.volume = noteHitVolume;
			noteHitSounds = new FlxTypedGroup<FlxSound>();
			add(noteHitSounds);
		}

		if (curSong == "Finale")
		{
			adaptiveMusicGroup1 = new FlxTypedGroup<FlxSound>();
			add(adaptiveMusicGroup1);
			adaptiveMusicGroup2 = new FlxTypedGroup<FlxSound>();
			add(adaptiveMusicGroup2);
			adaptiveMusicGroup3 = new FlxTypedGroup<FlxSound>();
			add(adaptiveMusicGroup3);
			adaptiveMusicGroup4 = new FlxTypedGroup<FlxSound>();
			add(adaptiveMusicGroup4);
			adaptiveMusicGroup5 = new FlxTypedGroup<FlxSound>();
			adaptiveMusicGroup5.volume = 0.1;
			add(adaptiveMusicGroup5);
		}
	}
	...
	//Yup it was!
	//a
	function startNoCountdown():Void
	{
		trace('started w/o countdown');
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			if (curSong != "Mic Test" && curSong != "Finale")
			{
				if (health < 1)
			...
			else if (curSong != "Mic Test" && curSong == "Finale")
			{
				if (health < 1)
				{
					adaptiveMusicGroup1.volume = health;
					adaptiveMusicGroup2.volume = health;
					adaptiveMusicGroup3.volume = health;
					adaptiveMusicGroup4.volume = health;
					adaptiveMusicGroup5.volume = health * -1 + 1;
				}
				else if (health > 1 && FlxG.sound.music.volume < 1)
				{
					adaptiveMusicGroup1.volume = 1;
					adaptiveMusicGroup2.volume = 1;
					adaptiveMusicGroup3.volume = 1;
					adaptiveMusicGroup4.volume = 1;
					adaptiveMusicGroup5.volume = 0.1;
				}
			}
		}
		...
		if (hurtVignette.alpha > 0)
			hurtVignette.alpha -= (Conductor.crochet / cityLightTimeShit * 2.5) * FlxG.elapsed;

		if (camShake && FlxG.save.data.distractions)
		{
			if (isEathquake)
			{
		...
		if (health <= 0 && !cannotDie && !resultsPresent && !inCutscene)
		{
			if (!usedTimeTravel) 
			{
				literallyFuckingDie();
		...
		{
			if (FlxG.keys.justPressed.R && !resultsPresent)
			{
				literallyFuckingDie();
			}
		}
		...
		else
		{
			...
			else
			{
				...
				if (FlxG.save.data.scoreScreen && !resultsPresent) 
				{
					camHUD.visible = true;
					openSubState(new ResultsScreen());
					if (SONG.song.toLowerCase() == "technical difficulties")
					{
						orangeTechDiff = true;
						resultsPresent = true;
					}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			if (!PlayStateChangeables.botPlay || loadRep)
			{
				...
				if (!FlxG.save.data.notesplash && FlxG.save.data.distractions)
				{
					if (!curStage.startsWith('school'))
					{
				...
				else if (!FlxG.save.data.notesplash && !FlxG.save.data.distractions)
				{
					if (!curStage.startsWith('school'))
					{
...
//					trace(daNote.sustainActive);
					goodNoteHit(daNote);
					if (FlxG.save.data.notesfx && FlxG.save.data.distraction && allowNoteHitSounds)
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('Note_Sustain'), 0.25);
					}
					else if (FlxG.save.data.notesfx && !FlxG.save.data.distraction && allowNoteHitSounds)
					{
						//For Sustain Notes
						FlxG.sound.play(Paths.sound('NoteChill_Sustain'), 0.2);
					}
				}
			});
		}
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (daNote != null)
			{
				if (!daNote.isSustainNote && daNote.noteStyle == 'normal')
					songScore -= 10;
			}
	...
	function playNoteHitSound(note:Note):Void
	{
		...
		if (!PlayStateChangeables.botPlay || loadRep || FlxG.save.data.distractions)
		{
			//Converted from nested if-else statements to a switch one! You're welcome!
			switch (rating)
			{
				case 'sick':
					//For Sick Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('Note_Sick'), 0.9);
				case 'good':
					//For Good Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('Note_Good'), 0.9);
				case 'bad':
					//For Bad Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('Note_Bad'), 0.9);
				case 'shit':
					//For Shit Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('Note_Crap'), 0.9);
			}
		}
		else if (!PlayStateChangeables.botPlay || loadRep || !FlxG.save.data.distractions)
		{
			//Converted from nested if-else statements to a switch one! You're welcome (again!)
			switch (rating)
			{
				case 'sick':
					//For Sick Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('NoteChill_Sick'), 0.9);
				case 'good':
					//For Good Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('NoteChill_Good'), 0.9);
				case 'bad':
					//For Bad Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('NoteChill_Bad'), 0.9);
				case 'shit':
					//For Shit Rating
					noteHitSounds = FlxG.sound.load(Paths.sound('NoteChill_Crap'), 0.9);
			}
		}
		else
		{
			noteHitSounds = FlxG.sound.load(Paths.sound('Note_Botplay'), 0.8);
		}
		noteHitSounds.play(true);
	}
	...
	function literallyFuckingDie():Void
	{
		...
		{
			FlxG.sound.play(Paths.sound('GitarooFail'), 1.5);
		}
	}
	...
	function camFollowShit()
	{
		...
		if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		{
			...
			{
				if (curSong == 'Mic Test')
					FlxTween.tween(FlxG.camera, {zoom: 1.5}, (Conductor.stepCrochet * 4 / 750), {ease: FlxEase.elasticInOut});
		...
		if (camFollow.x != boyfriend.getMidpoint().x - 100 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		{
			...
			{
				if (curSong == 'Mic Test')
					FlxTween.tween(FlxG.camera, {zoom: 1.2}, (Conductor.stepCrochet * 4 / 650), {ease: FlxEase.elasticInOut});
	...
	override function beatHit()
	{
		...
		if (generatedMusic)
		{
			...
			if (curSong != "Mic Test")
			{
				if (health < 1 && !midsongCutscene && allowHeartBeatSounds)
				{
					lowHealthSoundVolume = health * -1 + 1;
					...
					if (Conductor.bpm <= 300)
					{
						...
						{
							FlxG.sound.play(Paths.sound('lowHP'), lowHealthSoundVolume);
		...
		if (FlxG.save.data.camzoom && camZooming && !resultsPresent)
		{
			...
			if (curSong.toLowerCase() == 'retaliation' && FlxG.camera.zoom < 1.35)
			{
			...
			if (curSong.toLowerCase() == 'up and beyond')
			{
				if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % 4 == 1)
				{
		...
		if (!resultsPresent && !PlayStateChangeables.Optimize)
		{
			switch (curSong)
			{
				case 'Mic Test':
					...
					if (curBeat % 8 == 3 && curBeat > 122 && curBeat < 150)
					{
						if (accuracy > 30)
						{
							boyfriend.playAnim('hey', true);
				...
				case 'Awkward Pauses':
					switch (curBeat)
					{
						case 254:
							...
							FlxTween.tween(FlxG.camera, {zoom: 1}, 0.3, {ease: FlxEase.quadIn});
				...
				case 'Up and Beyond':
					if (curBeat % 4 == 3 && curBeat > 193 && curBeat < 257)
					{
					...
					if (curBeat % 4 == 3 && curBeat > 389 && curBeat < 513)
					{
						if (FlxG.save.data.camzoom && FlxG.save.data.flashing)
						{
							FlxG.camera.zoom += 0.015;
				...
				case 'Playing With Fire':
					if (curBeat % 2 == 1 && curBeat > 276 && curBeat < 304)
					{
					...
					switch (curBeat)
					{
						...
						case 176:
							...
							if (deaths <= 0 && !hasReset)
							{
								new FlxTimer().start(0.025, function(tmr:FlxTimer)
								{
									avoidBulletTipGraphic.alpha += 0.085;
									if (avoidBulletTipGraphic.alpha < 1)
									{
										tmr.reset(0.025);
									}
									else
						...
						case 180:
							...
							if (deaths <= 0 && !hasReset)
							{
								new FlxTimer().start(0.025, function(tmr:FlxTimer)
								{
									avoidBulletTipGraphic.alpha -= 0.09;
									if (avoidBulletTipGraphic.alpha > 0)
									{
										tmr.reset(0.025);
									}
									else
						...
						| 433 | 468 | 472 | 477 | 480 | 484 | 488 | 492 | 496 | 531 | 535 | 546 | 549 | 552 | 562:
							dad.playAnim('gunLOAD', true);
							if (!vignette.animation.curAnim.name.startsWith("warning"))
				...
				case 'Finale'://ADAPTIVE MUSIC!! WOOOOO!!!
					...
					if (curBeat % 16 == 0)
					{
						adaptiveMusicGroup1 = FlxG.sound.load(Paths.soundRandom('finaleAdaptiveMus/drums_', 0, 3), 0.8);
						adaptiveMusicGroup1 = FlxG.sound.load(Paths.soundRandom('finaleAdaptiveMus/taiko_', 0, 5), 0.8);
						adaptiveMusicGroup1.play();
					}
					...
					if (curBeat % 32 == 0)
					{
						if (dangerLevel < 50)
						{
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/choir_0'), 0.8);
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/hats_0'), 0.8);
							if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
								adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/woodwinds'), 0.8);
						}
						else
						{
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/choir_1'), 0.8);
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/hats_1'), 0.8);
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/violin'), 0.8);
						}
						if (enemyHealth < 30 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/slayer_0'), 0.8);
						else
						{
							adaptiveMusicGroup2 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/slayer_1'), 0.8);
							if (dangerLevel < 50)
								dangerLevel = 50;
						}
						adaptiveMusicGroup2.play();
					}
					//Cymbals/Hats
					if (curBeat % 64 == 0)
					{
						adaptiveMusicGroup3 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/stab'), 0.9);
						if (dangerLevel < 50 && PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
							adaptiveMusicGroup3 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/retal-hats_0'), 0.9);
						else
							adaptiveMusicGroup3 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/retal-hats_0'), 0.9);
						adaptiveMusicGroup3.play();
					}
					//Bells, and Pads
					if (curBeat % 64 == 32)
					{
						if (dangerLevel < 50)
						{
							adaptiveMusicGroup4 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/bells_0'), 0.9);
							if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
								adaptiveMusicGroup4 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/pads_0'), 0.9);
						}
						else
						{
							adaptiveMusicGroup4 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/bells_1'), 0.9);
							adaptiveMusicGroup4 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/pads_1'), 0.9);
						}
						adaptiveMusicGroup4.play();
					}
					if (curBeat % 128 == 0)
					{
						adaptiveMusicGroup5 = FlxG.sound.load(Paths.sound('finaleAdaptiveMus/danger_loop'), 1);
						adaptiveMusicGroup5.play();
					}
			}
		}
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && curBeat % idleBeat == 0 && !midsongCutscene)
		...
		if (allowRandomGlitchFunction
			 && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
		{
			if (FlxG.save.data.distractions && !midsongCutscene && !boyfriend.animation.curAnim.name.startsWith('sing'))
			{
				lightningStrikeShit();
			}
		}
