...
class Ratings
{
    public static function GenerateLetterRank(accuracy:Float, ?keeledOver:Bool = false):String // generate a letter ranking
    {
        ...
        else
        {
            if (accuracy > 0)
            {
                ...
                {
                    if (PlayState.instance.misses == 0 && PlayState.instance.bads == 0 && PlayState.instance.slips == 0 && PlayState.instance.goods == 0) // Perfect (SICK) Full Combo
                        ranking = "PFC";
                    else if (PlayState.instance.misses == 0 && PlayState.instance.bads == 0 && PlayState.instance.slips == 0 && PlayState.instance.goods >= 1) // Good Full Combo (Nothing but Goods & Sicks)
                        ranking = "GFC";
                        ...
                        ranking = "DDCB";
                    else if (PlayState.instance.accuracy >= 70)
                        ranking = "CLEAR";
                    else
                        ranking = "RETRY";
                }
    ...
    public static function CalculateRanking(score:Int, NPS:Int, maxNPS:Int, accuracy:Float, ?keeledOver:Bool = false):String
    {
        return
         ...
         " | Misses:" + PlayState.instance.misses + 																				         // Misses/Combo Breaks
