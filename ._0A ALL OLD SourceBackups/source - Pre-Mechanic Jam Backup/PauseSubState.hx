...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		pauseMusic = new FlxSound().loadEmbedded(Paths.music(PlayState.instance.pauseMusicName), true, true);
		if (PlayState.instance.pauseMusicName != 'pauseScreen/holy_shit_the_stalemate_is_on_fire')
			pauseMusic.volume = 0;
		else
			pauseMusic.volume = 0.45;
		...
		for (i in 0...menuItems.length)
		{
			...
			item.y *= i;
			item.distancePerItem.set(30, 170);
	...
	override function update(elapsed:Float)
	{
		...
		#if cpp
		else if ((controls.LEFT_P || leftPcontroller) && !didCountdownShit)
		{
		...
		else if ((controls.RIGHT_P || rightPcontroller) && !didCountdownShit)
		{
			oldOffset = PlayState.songOffset;
			PlayState.songOffset += 1;
			sys.FileSystem.rename(songPath + oldOffset + '.offset', songPath + PlayState.songOffset + '.offset');
		...
		if (controls.ACCEPT && !didCountdownShit)
		{
			...
			switch (daSelected)
			{
				case "Resume":
					...
					if (doFancyCountdownShit && !didCountdownShit && PlayState.songStarted) //(FlxG.save.data.pauseCountdown)
						doCountdownShit();
					else
						close();
				case "Restart Song":
					...
					PlayState.showedLoadingIcon = true;
				...
				case "Skip Song":
					...
					if (PlayState.storyPlaylist.length > 1 && FlxG.save.data.weekUnlocked >= PlayState.storyWeek + 1)
					{
						...
						PlayState.instance.loadingIcon.alpha = 1;
					...
					{
						FlxG.sound.play(Paths.sound('scrollMenuFail'), 0.4);
						trace("Cannot Skip");
						skippedSong = false;
					}
				case "Exit to menu":
