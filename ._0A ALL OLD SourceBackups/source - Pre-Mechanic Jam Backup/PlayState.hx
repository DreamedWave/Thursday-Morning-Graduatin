...
class PlayState extends MusicBeatState
{
				...
				if (!PlayStateChangeables.Optimize)
				{
					switch(songLowercase)
					{
						...
						case "disagreement" | "retaliation" | "playing-with-fire":
							...
							if (swagCounter + 1 % idleBeat == 0 || dad.curCharacter == "priest-theborderpray" || dad.curCharacter == "table-default")
						...
						case "variegated-skylines":
							...
							if (gfSpeed > 0 && swagCounter + 1 % gfSpeed == 0)
								gf.dance();
	...
	override function stepHit()
	{
		...
		if (healthBar.percent < 50 && naturalDeaths > 0 && targetHealth > 0)
	...
	private function altCamTween(time:Float = 0, twnType:String = 'smoothStep')
	{
		if (time == 0)
	...
	override function beatHit()
	{
		...
		{	
			...
			if (SONG.notes[Std.int(curStep / 16)] != null)
			{
				...
				if (songStarted && !midsongCutscene)
				{
					if (gfSpeed > 0 && curBeat % gfSpeed == 0)
						gf.dance();

					//(IDK???)> (╯°□°）╯︵ ┻━┻
					if (((boyfriend.animation.curAnim.name != 'idleAfterSing' && !boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.startsWith('cheer') || boyfriend.animation.curAnim.finished) && (boyfriend.animation.curAnim.name != "hurt" && !boyfriend.animation.curAnim.name.startsWith("dodge") || boyfriend.animation.curAnim.finished)) && curBeat % idleBeat == 0)
					{
						boyfriend.playAnim('idle', idleToBeat);
						if (isBFTurn)
							singFollowOffset = [0, 0];
						...
						case "Disagreement":
							if (FlxG.save.data.camzoom && camZooming)
							{
								if (curBeat % 4 == 0)
								{
									...
									camHUD.zoom += 0.015;
									if (camZoomUsesTween)
										altCamTween(Conductor.crochet * 3 / 1000, 'elastic');
								}
