...
class MainMenuState extends MusicBeatMenu
{
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			else
			{
				...
				if (i == 0)
				{
					arrowTween = FlxTween.tween(menuArrow, {x: 0}, 0.5 + (curSelected * 0.15), {type: ONESHOT, ease: FlxEase.expoOut, startDelay: 0.5 + (curSelected * 0.15), onComplete: function(flxTween:FlxTween)
						{
							arrowTween = null;
						}
					});
	...
	{
		if (FlxG.sound.music.volume < 0.65 && !preventMusicVolumeFixing)
		...
		if (!selectedSomethin)
		{
			...
			else if ((controls.ACCEPT || FlxG.mouse.justPressed && FlxG.mouse.overlaps(menuItems) && Main.isFocused) && !movedBack)
			{
				...
				if (menuItems.members[curSelected].intTween != null)
					menuItems.members[curSelected].intTween.cancel();
				menuItems.members[curSelected].animation.play('selected');
				menuItems.members[curSelected].x = 30;
		...
		for (faith in bgGroup)
			faith.y = FlxMath.lerp(bgGroupTargetY, faith.y, PlayStateHelpers.calculateLerpTime(elapsed * 10, 1, 0, 1));

	}

	var preventExitingTEMP:Bool = false;
