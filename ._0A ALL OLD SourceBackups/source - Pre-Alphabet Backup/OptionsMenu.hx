...
class OptionsMenu extends MusicBeatMenu
{
	...
	var options:Array<OptionCategory> = [
		...
		new OptionCategory("Accessibility", [
			...
			new DistractionsAndEffectsOption("Toggle in-game events and/or animations that could hinder your gameplay."),
			...
			new SubtitlesCenteredOption("If on, subtitles are automatically centered rather than being aligned to the left."),
			...
			new GhostTapOption("If on, pressing an input with no note doesn't give you a penalty."),
			new ShowStrumLine("Toggle the background for your strumline in-game."),
			new ScrollSpeedOption("Change the chart's scroll speed multiplier.\n(Example: 0.5 halves speed, 2 doubles speed, etc.)\n([LEFT/RIGHT] to change)")
		...
		new OptionCategory("Graphics and Video", [
			...
			new NightModeOption("Darkens UI and gameplay to prevent eye strain during night."),
			new AutoNightModeOption("If on, dark-mode triggers automatically based the time of day."),
			new NoteSplashingEffectsOption("Toggle the effects when you hit a 'SICK!!' or miss a note."),
		...
		new OptionCategory("Sounds and Music", [
			...
			new HitSFXTypeOption("Change how you want your note hit sounds to sound like. ([LEFT/RIGHT] to choose)"),
			new HitSFXVolOption("Change the volume of note hit sounds. ([LEFT/RIGHT] to change, [SHIFT] to fast)"), //Remove this and replace with SFX Volume
		...
		new OptionCategory("Performance and Caching", [
			...
			new Optimization("Simplifies gameplay by removing everything except your strumline and a still background image."),
	...
	override function create()
	{
		...
		offsetNumberShit = new FlxText(5, -40, FlxG.width - 5, "Offset ([LEFT/RIGHT], [SHIFT] to slow): " + HelperFunctions.truncateFloat(FlxG.save.data.offset, 2) + '${FlxMath.getDecimals(HelperFunctions.truncateFloat(FlxG.save.data.offset, 2)) != 0 ? '' : '.0'} ms');
		...
		blackBorder1 = new FlxSprite(-30, -40).makeGraphic((Std.int(FlxG.width + 100)), Std.int(offsetNumberShit.height), FlxColor.BLACK);
		...
		FlxTween.tween(offsetNumberShit,{y: 3}, 1.1 ,{ease: FlxEase.elasticInOut});
		FlxTween.tween(blackBorder1,{y: 1}, 1 , {ease: FlxEase.elasticInOut, onComplete:
	...
	override function update(elapsed:Float)
	{
		...
		if (acceptInput)
		{
			...
			else if ((controls.BACK || FlxG.mouse.justPressedRight) && movedBack && !isCat && !DisclaimerScreen.wentOptions)
			{
			...
			if (isCat)
			{
				...
				else
				{
					...
					offsetNumberShit.text = "Offset ([LEFT/RIGHT], [SHIFT] to slow): " + HelperFunctions.truncateFloat(FlxG.save.data.offset, 2) + '${FlxMath.getDecimals(HelperFunctions.truncateFloat(FlxG.save.data.offset, 2)) != 0 ? '' : '.0'} ms';
					descriptionShit.text = currentDescription;
				}
			}
			else
