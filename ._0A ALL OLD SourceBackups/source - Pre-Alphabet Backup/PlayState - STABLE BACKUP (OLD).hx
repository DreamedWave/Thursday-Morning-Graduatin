...
class PlayState extends MusicBeatState
{
	...
	override function onFocus():Void
	{
		if (songStarted && generatedMusic && !endedSong && FlxG.save.data.pauseOnUnfocus && !paused)
		{
			allowHealthModifiers = true; //infinite loop prevention
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			note.finnaBeKilled = true;
			if (!note.isSustainNote || note.delayedDeath && !note.childBehindParent)
			{
				note.kill();
				notes.remove(note, true);
				note.destroy();
	...
	override function stepHit()
	{
		...
		if (doStrumLineBGTweening)
		{
			...
			else
			{
				if (strumLineBGTween != null)
					strumLineBGTween.cancel();
				strumLineBGTween = FlxTween.tween(grpStrumLine, {alpha: 0.5}, Conductor.crochet * 2 / 1000, {type: ONESHOT, ease: FlxEase.quadOut, startDelay: Conductor.crochet * 2 / 1000, onComplete:
					function (twn:FlxTween)
					{
						strumLineBGTween = null;
					}
