...
class TitleState extends MusicBeatMenu
{
	...
	override function update(elapsed:Float)
	{
		...
		if ((pressedEnter || !FlxG.mouse.overlaps(closeButton) && FlxG.mouse.justPressed && !FlxG.mouse.justPressedRight) && !pressedBackspace && skippedIntro)
		{
			//Go to Main Menu
			pressedAnything(1);
		}
		else if((pressedBackspace || FlxG.mouse.overlaps(closeButton) && FlxG.mouse.justPressed || FlxG.mouse.justPressedRight) && !pressedEnter && !transitioning && skippedIntro)
		{
			//Exit
			pressedAnything(2);
		}
		else if ((pressedEnter || FlxG.mouse.justPressed) && !skippedIntro && initialized)
		{
			//SkipIntro
			pressedAnything(0);
		}
