...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addOptionsUI()
	{
		...
		var check_mute_voc = new FlxUICheckBox(10, 120, null, null, "Mute Voices", 100);
		check_mute_voc.checked = false;
		...
		var check_mute_miscs = new FlxUICheckBox(10, 150, null, null, "Mute Miscs", 100);
		...
		tab_options.add(check_mute_voc);
		if (PlayState.hasMiscs)
			tab_options.add(check_mute_miscs);
	...
	override function update(elapsed:Float)
	{
		...
		noteTypeText.text = "Note-type: " + noteStyle + " (Normal [0], Mine [1], Trigger [2])";
			...
			curRenderedNotes.forEach(function(note:Note)
			{
				if (FlxG.sound.music.playing)
				{
					if (strumLine.overlaps(note))
					{
						...
						{
							claps.push(note);
							FlxG.sound.play(Paths.sound('Note_Botplay'));
						}
					}
				}
			});
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n != null)
			section.sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.noteStyle]);
		else
