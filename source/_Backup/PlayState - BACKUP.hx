...
[X]   Add loading bars to asynchlloops? hehehehe hehehehe ehhehe
...
class PlayState extends MusicBeatState
{
	...
	static var naturalDeaths:Int = 0; //For death pity
	...
	**/
	...
	var sectionsFCd:Int = 1;
	var sectionsToFC:Int = 2; //Threshold for how many sections you want until it counts up one intimidation point
	...
	override public function create():Void
	{
		...
		switch (songLowercase)
		{
			...
			case 'finale' | 'sudden-confrontation' | 'sprouting-irritation' | 'striking-tribulation':
				...
				if (songLowercase != 'finale')
				{
					...
					intimidationPTSBar.antialiasing = true;
					...
					intimidationBeatsBar.antialiasing = true;
		...
		resetText = new FlxText(0, 0, 0, "[Double tap [R] to Reset]", 45);
	...
	function desperationIntro():Void
	{
		...
		camGame.followLerp = camFollowSpeed * (60 / (cast(Lib.current.getChildAt(0), Main)).getFPS());
	...
	function startCountdown(altSuffix:String = ""):Void
	{
		...
		if (health != 1 && prevHealth != 1)
			prevHealthTwn = FlxTween.tween(this, {health: 1}, Conductor.crochet * 3.75 / 1000, {ease: FlxEase.expoOut});
	...
	function handleInput(event:KeyboardEvent):Void // this actually handles press inputs
	{
		...
		if (startedCountdown && key > -1 && FlxG.keys.checkStatus(eventKey, JUST_PRESSED))
		{
			...
			else if (songStarted && allowHealthModifiers && nearestNoteUpcoming)
			{
				...
				else
				{
					...
					if (mashPresses > mashPressThreshold)
					{
						mashing = true;
						accuracy -= 0.2;
						targetHealth += calculateHealth(storyDifficulty, 9);
						songScore -= 20;
						updateAccuracy();
						#if debug
	...
	private function styleCheck():Void
	{
		...
		if (goodStyleHits >= 1)
		{
			...
			targetHealth += calculateHealth(storyDifficulty, 14, (FlxMath.isOdd(goodStyleHits) ? health : health / 2), accuracy, goodStyleHits);
		}
		goodStyleHits = 0;
		improvStyleHits = 0;
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (didCachingShits)
		{
			...
			if (controls.PAUSE && !video.isPlaying && crashPrevBool && canPause && !paused && (FlxG.sound.music.time < musicTimeCusp || !FlxG.sound.music.playing))
				pauseGame();
							...
							if (!daNote.delayedDeath && daNote.enabled)
							{
								if (daNote.mustPress && !daNote.withinCompensation && !PlayStateChangeables.botPlay)
								{
									if (!daNote.isSustainNote)
									{
										...
										{
											case 'mine':
												if (stageOverlay != null && !PlayStateChangeables.Optimize)
													stageOverlay.animation.play('static');
											...
											default:
												if (allowHealthModifiers && !daNote.withinCompensation && hurtVignette.alpha < 0.2)
												{
													if (daNote.isParent)
													{
														...
														targetHealth += calculateHealth(storyDifficulty, 1, targetHealth, accuracy);
														totalNotesHit -= 1;
													...
													else
													{
														...
														targetHealth += calculateHealth(storyDifficulty, 0, targetHealth, accuracy);
														vocals.volume = 0;
													}
													noteMiss(daNote.noteData, daNote);
												}
									...
									else
									{
										if (!daNote.isParent && !daNote.wasGoodHit && allowHealthModifiers && !daNote.withinCompensation && hurtVignette.alpha < 0.2)
										{
											if (daNote.sustainActive && daNote.spotInLine != daNote.parent.children.length)
											{
												...
												targetHealth += calculateHealth(storyDifficulty, 2, targetHealth, accuracy);
												...
												targetHealth += calculateHealth(storyDifficulty, 3, targetHealth, accuracy);
										}
									}
			...
			if (FlxG.keys.justPressed.SPACE && skipActive && generatedMusic && startedCountdown && curBeat >= 0)
			{
				...
				if (songLowercase == 'mic-test')
				{
					...
					if (tutorialText != null)
					{
						...
						{
							for (i in 0...tutorialText.members.length)
							{
			...
			if (camZooming && !camZoomUsesTween)
			{
	...
	function getFuckingShot():Void
	{
		dad.playAnim('gunSHOOT', true);
		if (!PlayStateChangeables.Optimize)
			if (stageOverlay != null && stageOverlay.animation.curAnim.name.toLowerCase() == 'warning')
				stageOverlay.animation.play('static');
		...
		if (allowHealthModifiers)
		{
			...
			if (timesShot < 3 && timesClutched <= 5 + naturalDeaths - 2)
			{
			...
			{
				if (timesClutched < 5 + naturalDeaths - 2 && timesClutched < 10) //Caps clutching to 10 so that it dont go up infinitely LMAO
				{
	...
	{
		if (PlayState.instance.accuracy < 70 && !cannotDie && !PauseSubState.skippedSong)
		{
			literallyFuckingDie();
			return;
		}
		...
		switch (type)
		{
			...
			default:
				...
				if (allowHealthModifiers && !daNote.withinCompensation)
				{
					...
					maxSongScore += calculateScore(daRating);
					...
					switch (daRating)
					{
						...
						case 'shit':
							if (lagCompIcon.alpha < 0.1)
							{
								...
								if (hurtVignette.alpha < 0.2)
									targetHealth += calculateHealth(storyDifficulty, 4, targetHealth, accuracy);
								timingColour = FlxColor.RED;
							}
						case 'bad':
							daRating = 'bad';
							//Health Drain
							if (hurtVignette.alpha < 0.2)
								targetHealth += calculateHealth(storyDifficulty, 5, targetHealth, accuracy);
							bads++;
							if (FlxG.save.data.accuracyMod == 0)
								if (mashPresses <= mashPressThreshold)
									totalNotesHit += 0.50;
						...
						case 'good':
							...
							daRating = 'good';
							//Health Gain
							targetHealth += !mashing ? calculateHealth(storyDifficulty, 6, targetHealth, accuracy) * (isJack ? 1.25 : 1) : calculateHealth(storyDifficulty, 6, targetHealth, accuracy) / (4 * mashPresses - mashPressThreshold);
							goods++;
							if (FlxG.save.data.accuracyMod == 0)
								if (mashPresses <= mashPressThreshold)
									totalNotesHit += 0.75;
						...
						case 'sick':
							...
							//Health Gain
							targetHealth += !mashing ? calculateHealth(storyDifficulty, 7, targetHealth, accuracy, hurtVignette.alpha) * (isJack ? 1.25 : 1) : calculateHealth(storyDifficulty, 7, targetHealth, accuracy, hurtVignette.alpha) / (4 * mashPresses - mashPressThreshold);
							if (FlxG.save.data.accuracyMod == 0)
								if (mashPresses <= mashPressThreshold)
									totalNotesHit += 1;
							sicks++;
	...
	private function enemyNoteMiss(noteDirection, isSustain:Bool = false, ?curEnemyNoteCheck:Int = 0):Void
	{
		...
		if (!isSustain)
		{
			...
			if (healthBar.percent < 85)
				targetHealth += calculateHealth(storyDifficulty, 10, targetHealth, accuracy);
		}
		else
		{
			if (curEnemyNote == curEnemyNoteCheck)
			{
				...
				if (healthBar.percent < 85)
					targetHealth += calculateHealth(storyDifficulty, 11, targetHealth, accuracy) / 2;
			}
		}
		dad.holdTimer = 0;
	}
				...
				case "Sudden Confrontation" | "Sprouting Irritation" | "Striking Tribulation":
					...
					if (healthBar.percent >= 25 && allowHealthModifiers && !note.withinCompensation)
					{
						if (!note.isSustainNote)
							...
							targetHealth -= calculateHealth(storyDifficulty, 11, targetHealth, accuracy);
					}
				default:
					if (SONG.needsVoices)
						vocals.volume = vocalsVolume;
	...
	function goodNoteHit(note:Note, ?isJack:Bool = false):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			else
			{
				...
				if (allowHealthModifiers)
					targetHealth += !mashing ? calculateHealth(storyDifficulty, 8, targetHealth, accuracy) : calculateHealth(storyDifficulty, 8, targetHealth, accuracy) / 4;
				songScore += 5;
				maxSongScore += 5;
				vocals.volume = vocalsVolume;
				if (mashPresses <= mashPressThreshold)
			...
			switch (note.noteType)
			{
				...
				case 'trigger':
					...
					targetHealth += calculateHealth(storyDifficulty, 12, targetHealth, accuracy);
	...
	override function stepHit()
	{
		...
		if (healthBar.percent < 50 && naturalDeaths - storyDifficulty > 0 && targetHealth > 0)
			targetHealth += naturalDeaths * 0.001;
			...
			switch (curSong)
			{
				...
				case "Desperation":
					switch (curBeat)
					{
						...
						case 188:
							changeSubtitles("(that's a little too much for me..?)", 0xFF4adaff);
						case 190:
							clearSubtitles();
						case 192:
							changeSubtitles("LORD. PLEASE. HELP. 'US'.", 0xFF5ce766);
	...
	private function calculateScore(daRating:String):Int
    {
        switch(daRating)
            {
                case 'miss':
    ...
    private function calculateHealth(difficulty:Int, hpDrainType:Int, ?health:Float = 1, ?accuracy:Float = 90, ?externalValue:Float = 0)
    {
        ...
        switch (hpDrainType)
        {
            ...
            case 9:
                feedbackHealth = ((difficulty + 1) / accuracy) * 0.05;
            //Enemy Sing
            case 10:
                switch (difficulty)
                {
	...
	override function beatHit()
	{
		...
		{	
			...
			if (SONG.notes[Std.int(curStep / 16)] != null)
			{
				...
				if (songStarted && !midsongCutscene && !endedSong)
				{
							...
							case 'Striking Tribulation':
								switch (curBeat)
								{
									...
									case 864:
										...
										dadFollowOffset[1] += 60;
										dummyBlackScreen.alpha = 1;
										camGame.shake(0.025, 0.3, true, true);
									case 896:
										camFollowSpeed = 0.01;
					...
					switch (curSong)
					{
						...
						case "Variegated Skylines":
							if (FlxG.save.data.camzoom && camZooming)
							{
								...
								if (curBeat % 4 == 2 && curBeat > 96 && curBeat < 130 || curBeat > 192 && curBeat < 256)
								{
									...
									camHUD.zoom += 0.015;
									if (!PlayStateChangeables.Optimize)
										if (stageOverlay != null && FlxG.save.data.flashing && FlxG.save.data.distractions)
											stageOverlay.animation.play('singleFlash');
								}	
								...
								if (curBeat > 352 && curBeat < 416 && curBeat != 383)
								{
									...
									camHUD.zoom += 0.0155;
									if (PlayStateChangeables.Optimize)
										if (stageOverlay != null && curBeat % 2 == 0 && FlxG.save.data.flashing && FlxG.save.data.distractions)
											stageOverlay.animation.play('singleFlash');
								}
							}
							else
							{
								...
								if (PlayStateChangeables.Optimize)
									if (stageOverlay != null && curBeat % 4 == 2 && curBeat > 96 && curBeat < 130 || curBeat > 192 && curBeat < 256 && FlxG.save.data.flashing && FlxG.save.data.distractions)
										stageOverlay.animation.play('singleFlash');

								if (PlayStateChangeables.Optimize)
									if (stageOverlay != null && curBeat > 352 && curBeat < 416 && curBeat != 383 && curBeat % 2 == 0 && FlxG.save.data.flashing && FlxG.save.data.distractions)
										stageOverlay.animation.play('singleFlash');
							}
						...
						case "Sudden Confrontation" | "Sprouting Irritation":
							...
							else
							{
								...
								{
									if (missesInSection <= 0 && (notesHitInSection >= 8 || isBFTurn))
									{
										if (sectionsFCd < sectionsToFC)
										{
											sectionsFCd++;
						...
						case "Striking Tribulation":
							...
							else
							{
								if (curBeat % 16 == 0 && curBeat > 8)
								{
									if (missesInSection <= 0 && notesHitInSection > 0)
									{
										if (sectionsFCd < sectionsToFC)
										{
											sectionsFCd++;
