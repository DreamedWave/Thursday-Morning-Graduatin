...
class KeyBindMenu extends FlxSubState
{
	...
	{	
       ...
       yikers2 = FlxTween.tween(blackBox, {alpha: 0.75}, 0.75, {ease: FlxEase.expoOut, onComplete: 
	...
	override function update(elapsed:Float)
	{
        ...
        switch(state)
        {
            case "select":
                ...
                {
                    FlxG.sound.play(Paths.sound('optionsToggleOn'));
                    state = "input";
                }
    ...
    function reset()
    {
        FlxG.sound.play(Paths.sound('keybindsCancel'), 0.7);
        for(i in 0...5)
        {
            keys[i] = defaultKeys[i];
        }
    ...
    function quit()
    {
        FlxG.sound.play(Paths.sound('keybindsChange'), 0.7);
        state = "exiting";
        remove(wipTxt);
    ...
    function addKeyGamepad(r:String){
        ...
        if(shouldReturn){
            ...
            FlxG.sound.play(Paths.sound('scrollMenu3'));
        }
        else{
            gpKeys[curSelected] = tempKey;
            lastKey = r;
