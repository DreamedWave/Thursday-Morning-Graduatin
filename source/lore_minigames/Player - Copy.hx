...
class Player extends FlxSprite
{
	...
	public var defaultMaxVelocityX:Float = 100;
	...
	override function update(elapsed:Float)
	{
		...
		{
			//Don't walk player if they're sliding
			if (!isPanicked)
			{
		...
		if(FlxG.keys.anyPressed([RIGHT, D]))
		{
			...
			else if (!isSneaking)
			{
				...
				if (isTouching(FLOOR)) //Speed on Ground
					acceleration.x = maxVelocity.x * 5;
				else //Speed on Air
					acceleration.x = maxVelocity.x * 2.5;
			}
			else
			{
				//Slide Cancel?	
				if (facing == LEFT && isSneaking)
					stopSliding();
				
				if (isTouching(FLOOR) && velocity.x < 20) //Speed on Ground
					velocity.x += 20;
			}
		...
		if (FlxG.keys.justPressed.SHIFT) //Sneaking
		{
			isSneaking = true;
			if (!isPanicked)
				{
			...
			else
			{
				if (resetTimer == null && canSlide)
				{
					...
					if (slideTimer != null)
						slideTimer.cancel();
					slideTimer = new FlxTimer().start(0.15, function(tmr:FlxTimer)
					{
