...
class Highscore
{
	...
	public static function saveWeekCombo(week:Int = 1, combo:String, ?diff:Int = 0):Void
	{
		if(!FlxG.save.data.botplay)
		{
			var daWeek:String = formatSong('week' + week, diff);
	...
	public static function getWeekCombo(week:Int, diff:Int):String
	{
		if (!songCombos.exists(formatSong('week' + week, diff)))
			setCombo(formatSong('week' + week, diff), '');

		return songCombos.get(formatSong('week' + week, diff));
	}
