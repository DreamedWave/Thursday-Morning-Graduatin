...
class Replay
{
	...
	public function SaveReplay(noteArray:Array<Dynamic>, judge:Array<String>, ana:Analysis)
	{
		...
		if (!FileSystem.exists("assets/temp/"))
			FileSystem.createDirectory("assets/temp/");

		#if sys
		File.saveContent("assets/temp/tempData-" + PlayState.SONG.song + "-time" + time + ".tempSongData", data);

		path = "tempData-" + PlayState.SONG.song + "-time" + time + ".tempSongData"; // for score screen shit
	...
	{
		#if sys
		trace('loading ' + Sys.getCwd() + 'assets/temp/' + path + ' replay...');
		try
		{
			var repl:ReplayJSON = cast Json.parse(File.getContent(Sys.getCwd() + "assets/temp/" + path));
			replay = repl;
		}
		catch(e)
		{
