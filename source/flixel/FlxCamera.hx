...
class FlxCamera extends FlxBasic
{
	...
	/**
	 * The values of `scroll` that get lerped
	 * Separated to prevent affecting camera effects such as camShake.
	 */
	 public var scrollLerp:FlxPoint = FlxPoint.get();
	...
	/**
	 * Previously internal, tracks whether fade effect is running or not.
	 */
	public var finishedFade:Bool = true;
	...
	/**
	 * Custom, a bool to check if the camera has the `shake()` effect or not
	 */
	public var isShakingCam:Bool = false;

	/**
	 * Custom & Internal, bool for whether `shake()` has a `decay` effect
	 */
	public var decayCamShake:Bool = false;

	/**
	 * Custom & Internal, bool for whether `shake()` is in the follow itself or the flashSprite
	 */
	 public var shakeFlashSprite:Bool = false;

	/**
	 * Custom & Internal, tweens the `shake()` effect's decay.
	 */
	 var shakeDecayFactor:Float = 1;
	
	/**
	 * Custom & Internal, tweens the `shake()` effect's decay.
	 */
	 var shakeDecayTween:FlxTween;
	...
	function transformObject(object:DisplayObject):DisplayObject
	{
		...
		object.x -= scrollLerp.x * totalScaleX;
		object.y -= scrollLerp.y * totalScaleY;
	...
	override public function destroy():Void
	{
		...
		scrollLerp = FlxDestroyUtil.put(scrollLerp);
	...
	public function updateScroll():Void
	{
		...
		scrollLerp.x = FlxMath.bound(scrollLerp.x, minX, (maxX != null) ? maxX - width : null);
		scrollLerp.y = FlxMath.bound(scrollLerp.y, minY, (maxY != null) ? maxY - height : null);
	...
	public function updateFollow(?elapsed:Float):Void
	{
		...
		else
		{
			...
			{
				if (targetX >= (scrollLerp.x + width))
				{
					_scrollTarget.x += width;
				}
				else if (targetX < scrollLerp.x)
				{
					_scrollTarget.x -= width;
				}

				if (targetY >= (scrollLerp.y + height))
				{
					_scrollTarget.y += height;
				}
				else if (targetY < scrollLerp.y)
				{
					_scrollTarget.y -= height;
				}
			}
	...
	var nextScrollShakeX:Float = 0;
	var nextScrollShakeY:Float = 0;
		...
		if (_fxShakeDuration > 0)
		{
			...
			else 
			{
				...
				else
				{
					...
					if (_fxShakeAxes != FlxAxes.X)
						flashSprite.y += (FlxG.random.float(-_fxShakeIntensity * 0.5 * height, _fxShakeIntensity * 0.5 * height) / 2) * (decayCamShake ? shakeDecayFactor : 1) * (FlxG.height / 720);
				}
			}
		}
	}
	...
	{
		scrollLerp.set(point.x - width * 0.5, point.y - height * 0.5);
		point.putWeak();
	}
	...
	{
		return scrollLerp.x + viewMarginX;
	}
	
	inline function get_viewY():Float
	{
		return scrollLerp.y + viewMarginY;
	}
	...
	{
		return scrollLerp.x + viewMarginRight;
	}
	
	inline function get_viewBottom():Float
	{
		return scrollLerp.x + viewMarginBottom;
	}
