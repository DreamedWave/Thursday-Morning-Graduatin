...
class FlxTween implements IFlxDestroyable
{
	 ...
	 * `manager` and can be specified if the `onComplete`
	 * callback will be called upon doing so.
	 *
	 * Yields control to the next chained Tween if one exists.
	 */
	public function cancel(?callbackOnComplete:Bool = false):Void
	{
	 ...
	 *
	 * The first tween of the chain's `onComplete` callback
	 * will be used if `callbackOnComplete` is set to true.
	 *
	 * @since 4.3.0
	 */
	public function cancelChain(?callbackOnComplete:Bool = false):Void
	{
		if (callbackOnComplete)
		{
			if (onComplete != null)
