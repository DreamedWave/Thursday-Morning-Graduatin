...
class GameplayCustomizeState extends MusicBeatMenu
{
    ...
    public override function create() 
    {
        ...
        if (!FlxG.save.data.downscroll)
        {
		    ...
		    FlxTween.tween(blackBorder,{y: FlxG.height - (blackBorder.height - 5), alpha: 0.7}, 1, {ease: FlxEase.elasticInOut});
        }
        else
        {
		    FlxTween.tween(text,{y: 5}, 1.1, {ease: FlxEase.elasticInOut});
		    FlxTween.tween(blackBorder,{y: -5, alpha: 0.7}, 1, {ease: FlxEase.elasticInOut});
        }
    ...
    override function update(elapsed:Float) 
    {
        ...
        text.text = "Click and drag around gameplay elements to customize their positions.\nPress [R] to reset, [ESCAPE] to save and exit.\nCurrent X: " + sick.x + " | Current Y:" + sick.y;
        ...
        if (controls.BACK && FlxG.mouse.visible)
        {
            ...
            FlxG.sound.play(Paths.sound('keybindsChange'));
	...
	private function generateStaticArrows(player:Int):Void
    {
        ...
        {
            // FlxG.log.add(i);
            var babyArrow:FlxSprite = new FlxSprite(0, strumLine.y + (!FlxG.save.data.downScroll ? -10 : 10));
