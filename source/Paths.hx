...
class Paths
{
	...
	static public function clearCurrentLevel()
	{
		currentLevel = null;
	}
	...
	/*inline static public function autosavedChart(file:String)
	{
		return getPath('temp/autosaved-charts/$file.json', TEXT, '');
	}*/
	...
	public static function clearStoredMemory(?clearExclusions:Bool = false):Void
	{
		...
		for (key in FlxG.bitmap._cache.keys())
		{
			...
			if (obj != null && !currentTrackedAssets.exists(key) && !key.startsWith('flixel.addons.transition'))  //removed && !dumpExclusions.contains(key)
			{
		...
		{
			if (!localTrackedAssets.contains(key) && !key.startsWith('flixel.addons.transition') && key != null)  //removed && !dumpExclusions.contains(key)
			{
	...
	{
		var data = CachingState.bitmapData.get(key);
		#if debug
