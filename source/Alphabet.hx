...
class Alphabet extends FlxSpriteGroup
{
	...
	var hasDummyHitbox:Bool = false;

	//public var textColor:FlxColor = 0xFFFFFF;

	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = true, ?menuItem:Bool = false, ?hitboxHeight:Float = 1, ?hitboxOffset:Float = 0)
	{
		super(x, y);
		...
		{
			hasDummyHitbox = true;
			var dummyHitbox:FlxSprite;
			if (bold)
				dummyHitbox = new FlxSprite(-100, 0).makeGraphic(FlxG.width * 2, Std.int(105), 0x000000); //0x000000
			else
				dummyHitbox = new FlxSprite(-100, 65).makeGraphic(FlxG.width * 2, Std.int(70), 0x000000);
			dummyHitbox.scale.y = hitboxHeight;
			dummyHitbox.updateHitbox();
			dummyHitbox.y -= Std.int(20 * hitboxHeight);
			dummyHitbox.y += hitboxOffset;
			dummyHitbox.alpha = 0.25;
			dummyHitbox.updateHitbox();
			add(dummyHitbox);
		}
	...
	public function makeIntoMenuItem(?hitboxHeight:Float = 1, ?hitboxOffset:Float = 0):Void
	{
		if (!isMenuItem)
			isMenuItem = true;
	...
	/*public function set_color(newColor:FlxColor)
	{
		clearLetters();
		createLetters(this.text);
		updateAlignment();
		textColor = newColor;
		return newColor;
	}*/
