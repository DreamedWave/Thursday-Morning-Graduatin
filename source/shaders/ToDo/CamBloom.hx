...
class BloomHandler
{
    ...
    public static function setAll(thresh:Float, int:Float, size:Float)
    {
        bloomShader.shader.data.threshold.value = [thresh];
        bloomShader.shader.data.intensity.value = [int];
		bloomShader.shader.data.blurSize.value = [size];
    }
}
