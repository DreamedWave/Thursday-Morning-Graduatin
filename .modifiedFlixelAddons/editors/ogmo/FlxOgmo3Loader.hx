//Modified by DreamedWave for TMG
...
class FlxOgmo3Loader
{
	 ...
	 */
	public function loadEntities(entityLoadCallback:EntityData->Int->Void, entityLayer:String = "entities"):Void
	{
		var entityAmount:Int = level.getEntityLayer(entityLayer).entities.length;
		for (entity in level.getEntityLayer(entityLayer).entities)
			entityLoadCallback(entity, entityAmount);
	}
